...
"""
Command-line interface to the OpenStack Patron API.
"""
...
import patronclient
import patronclient.auth_plugin
from patronclient import client
from patronclient import exceptions as exc
...
from patronclient.v2 import shell as shell_v2

DEFAULT_OS_COMPUTE_API_VERSION = "2"
DEFAULT_PATRON_ENDPOINT_TYPE = 'publicURL'
...
class SecretsHelper(object):
    ...
    def management_url(self):
        ...
        management_url = None
        try:
            block = keyring.get_password('patronclient_auth', self._make_key())
            if block:
...
class patronclientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(patronclientArgumentParser, self).__init__(*args, **kwargs)
...
class OpenStackComputeShell(object):
    ...
    def _discover_extensions(self, version):
        ...
        for name, module in itertools.chain(
            ...
            extension = patronclient.extension.Extension(name, module)
    ...
    def _discover_via_python_path(self):
        for (module_loader, name, _ispkg) in pkgutil.iter_modules():
            if name.endswith('_python_patronclient_ext'):
    ...
    def _discover_via_entry_points(self):
        for ep in pkg_resources.iter_entry_points('patronclient.extension'):
            name = ep.name
            module = ep.load()
    ...
    def do_bash_completion(self, _args):
        ...
        patron.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        ...
        print("... terminating patron client", file=sys.stderr)
        sys.exit(130)
