...
FAKE_ENV3 = {'OS_USER_ID': 'user_id',
             ...
             'OS_TENANT_ID': 'tenant_id',
             'OS_AUTH_URL': 'http://no.where/v2.0',
             'PATRON_ENDPOINT_TYPE': 'patronURL',
...
class ParserTest(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.parser = patronclient.shell.patronclientArgumentParser()
...
class ShellTest(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.nc_util = mock.patch(
            'patronclient.openstack.common.cliutils.isunauthenticated').start()
    ...
    def shell(self, argstr, exitcodes=(0,)):
        ...
        try:
            ...
            _shell = patronclient.shell.OpenStackComputeShell()
    ...
    def test_help(self):
        required = [
            ...
            '.*?^See "patron help COMMAND" for help on a specific command',
        ]
    ...
    @mock.patch('patronclient.client.Client')
    @requests_mock.Mocker()
    def test_patron_endpoint_type(self, mock_client, m_requests):
        self.make_env(fake_env=FAKE_ENV3)
        self.register_keystone_discovery_fixture(m_requests)
        self.shell('list')
        client_kwargs = mock_client.call_args_list[0][1]
        self.assertEqual(client_kwargs['endpoint_type'], 'patronURL')
    ...
    def test_os_endpoint_type(self, mock_client, m_requests):
        self.make_env(exclude='PATRON_ENDPOINT_TYPE', fake_env=FAKE_ENV3)
        self.register_keystone_discovery_fixture(m_requests)
        self.shell('list')
        client_kwargs = mock_client.call_args_list[0][1]
    ...
    @mock.patch('sys.argv', ['patron'])
    @mock.patch('sys.stdout', six.StringIO())
    @mock.patch('sys.stderr', six.StringIO())
    def test_main_noargs(self):
        # Ensure that main works with no command-line arguments
        try:
            patronclient.shell.main()
        except SystemExit:
            self.fail('Unexpected SystemExit')

        # We expect the normal usage as a result
        self.assertIn('Command-line interface to the OpenStack Patron API',
                      sys.stdout.getvalue())

    @mock.patch.object(patronclient.shell.OpenStackComputeShell, 'main')
