...
from patronclient.tests.unit.fixture_data import hosts as data
...
class HostsTest(utils.FixturedTestCase):
    ...
    def test_list_host(self):
        ...
        [self.assertIsInstance(h, hosts.Host) for h in hs]
        [self.assertEqual(h.zone, 'patron1') for h in hs]

    def test_list_host_with_zone(self):
        hs = self.cs.hosts.list('patron')
        self.assert_called('GET', '/os-hosts?zone=patron')
        [self.assertIsInstance(h, hosts.Host) for h in hs]
        [self.assertEqual(h.zone, 'patron') for h in hs]
