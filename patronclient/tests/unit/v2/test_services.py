...
class ServicesTest(utils.TestCase):
    ...
    def test_list_services(self):
        ...
        for s in svs:
            ...
            self.assertEqual('patron-compute', s.binary)
    ...
    def test_list_services_with_binary(self):
        svs = self.cs.services.list(binary='patron-cert')
        self.cs.assert_called('GET', '/os-services?binary=patron-cert')
        for s in svs:
            self.assertIsInstance(s, self._get_service_type())
            self.assertEqual('patron-cert', s.binary)
            self.assertEqual('host1', s.host)

    def test_list_services_with_host_binary(self):
        svs = self.cs.services.list(host='host2', binary='patron-cert')
        self.cs.assert_called('GET',
                              '/os-services?host=host2&binary=patron-cert')
        for s in svs:
            self.assertIsInstance(s, self._get_service_type())
            self.assertEqual('patron-cert', s.binary)
            self.assertEqual('host2', s.host)
    ...
    def test_services_enable(self):
        service = self.cs.services.enable('host1', 'patron-cert')
        values = self._update_body("host1", "patron-cert")
    ...
    def test_services_disable(self):
        service = self.cs.services.disable('host1', 'patron-cert')
        values = self._update_body("host1", "patron-cert")
    ...
    def test_services_disable_log_reason(self):
        service = self.cs.services.disable_log_reason(
            'compute1', 'patron-compute', 'disable bad host')
        values = self._update_body("compute1", "patron-compute",
                                   "disable bad host")
