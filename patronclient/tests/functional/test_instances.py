...
def volume_id_from_cli_create(output):
    ...
    The cli for Patron automatically routes requests to the volumes
    service end point. However the patron api low level commands don't
...
class TestInstanceCLI(base.ClientTestBase):
    def setUp(self):
        ...
        # they copied it from.
        self.client = patronclient.client.Client(
    ...
    def test_attach_volume(self):
        ...
        Patron endpoint. patronclient volumes support actually talks to
        ...
        self.patron('boot', params="--flavor %s --image %s %s --poll" %
        ...
        volume_id = volume_id_from_cli_create(
            self.patron('volume-create', params="1"))
        self.addCleanup(self.patron, 'volume-delete', params=volume_id)

        # allow volume to become available
        for x in xrange(60):
            volumes = self.patron('volume-list')
        ...
        # attach the volume
        self.patron('volume-attach', params="%s %s" % (name, volume_id))
        ...
        # clean up on success
        self.patron('volume-detach', params="%s %s" % (name, volume_id))
