...
from patronclient.openstack.common import uuidutils
...
@cliutils.arg(
    ...
    default=None,
    metavar='<flavor>',
    help=_("Name or ID of flavor (see 'patron flavor-list')."))
@cliutils.arg(
    '--image',
    default=None,
    metavar='<image>',
    help=_("Name or ID of image (see 'patron image-list'). "))
@cliutils.arg(
    '--image-with',
    default=[],
    ...
    action='append',
    metavar='<key=value>',
    help=_("Image metadata property (see 'patron image-show'). "))
@cliutils.arg(
    '--boot-volume',
    default=None,
    metavar="<volume_id>",
...
@cliutils.arg(
    '--key-name',
    default=os.environ.get('patronclient_DEFAULT_KEY_NAME'),
    metavar='<key-name>',
...
@cliutils.arg(
    '--tenant',
    # patron db searches by project_id
    dest='tenant',
    metavar='<tenant>',
    nargs='?',
...
def do_image_create(cs, args):
    ...
    if args.poll:
        ...
        # 'image-uploading' in Patron and clear the task-state once the VM
...
def _get_secgroup(cs, secgroup):
    # Check secgroup is an ID (patron-network) or UUID (neutron)
...
def do_service_list(cs, args):
    ...
    # NOTE(gtt): After https://review.openstack.org/#/c/39998/ patron will
    # show id in response.
...
def _quota_update(manager, identifier, args):
    ...
    if updates:
        ...
        # will be compatibile with old patron server
