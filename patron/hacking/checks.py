...
Guidelines for writing new hacking checks
 ...
 - Add test cases for each new rule to patron/tests/unit/test_hacking.py
...
virt_file_re = re.compile(r"\./patron/(?:tests/)?virt/(\w+)/")
virt_import_re = re.compile(
    r"^\s*(?:import|from) patron\.(?:tests\.)?virt\.(\w+)")
virt_config_re = re.compile(
    r"CONF\.import_opt\('.*?', 'patron\.virt\.(\w+)('|.)")
...
import_translation_for_log_or_exception = re.compile(
    r"(.)*(from\spatron.i18n\simport)\s_")
...
def import_no_db_in_virt(logical_line, filename):
    """Check for db calls from patron/virt

    As of grizzly-2 all the database calls have been removed from
    patron/virt, and we want to keep it that way.

    N307
    """
    if "patron/virt" in filename and not filename.endswith("fake.py"):
        if logical_line.startswith("from patron import db"):
            yield (0, "N307: patron.db import not allowed in patron/virt/*")
...
def no_import_translation_in_tests(logical_line, filename):
    """Check for 'from patron.i18n import _'
    N337
    """
    if 'patron/tests/' in filename:
...
def validate_log_translations(logical_line, physical_line, filename):
    ...
    if ("patron/tests" in filename or
