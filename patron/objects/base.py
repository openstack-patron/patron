...
from patron.objects import fields as obj_fields
    ...
    def __init__(cls, names, bases, dict_):
        ...
        for i, obj in enumerate(cls._obj_classes[obj_name]):
            if cls.VERSION == obj.VERSION:
                ...
                # Update patron.objects with this newer class.
            ...
            # an older version than anything we'e seen. Update patron.objects
...
class NovaObject(object):
    ...
    def _obj_from_primitive(cls, context, objver, primitive):
        ...
        self._context = context
        self.VERSION = objver
        objdata = primitive['patron_object.data']
        changes = primitive.get('patron_object.changes', [])
    ...
    def obj_from_primitive(cls, primitive, context=None):
        ...
        if primitive['patron_object.namespace'] != 'patron':
            # NOTE(danms): We don't do anything with this now, but it's
            # there for "the future"
            raise exception.UnsupportedObjectError(
                objtype='%s.%s' % (primitive['patron_object.namespace'],
                                   primitive['patron_object.name']))
        objname = primitive['patron_object.name']
        objver = primitive['patron_object.version']
    ...
    def _obj_make_obj_compatible(self, primitive, target_version, field):
        ...
        def _do_backport(to_version):
            ...
            if isinstance(obj, NovaObject):
                if to_version != primitive[field]['patron_object.version']:
                    obj.obj_make_compatible(
                        primitive[field]['patron_object.data'],
                        to_version)
                primitive[field]['patron_object.version'] = to_version
            elif isinstance(obj, list):
                for i, element in enumerate(obj):
                    element.obj_make_compatible(
                        primitive[field][i]['patron_object.data'],
                        to_version)
                    primitive[field][i]['patron_object.version'] = to_version
    ...
    def obj_make_compatible(self, primitive, target_version):
        ...
        of the object
        :raises: patron.exception.UnsupportedObjectError if conversion
    ...
    def obj_to_primitive(self, target_version=None):
        ...
        obj = {'patron_object.name': self.obj_name(),
               'patron_object.namespace': 'patron',
               'patron_object.version': target_version or self.VERSION,
               'patron_object.data': primitive}
        if self.obj_what_changed():
            obj['patron_object.changes'] = list(self.obj_what_changed())
        return obj
    ...
    # NOTE(danms): This is patron-specific, so don't copy this to o.vo
    ...
    def obj_make_compatible(self, primitive, target_version):
        ...
        for index, item in enumerate(self.objects):
            self.objects[index].obj_make_compatible(
                primitives[index]['patron_object.data'],
...
class NovaObjectSerializer(messaging.NoOpSerializer):
    ...
    def deserialize_entity(self, context, entity):
        if isinstance(entity, dict) and 'patron_object.name' in entity:
