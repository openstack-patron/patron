...
from patron.i18n import _, _LI
import patron.image.download.base as xfer_base
import patron.virt.libvirt.utils as lv_utils
...
#  This module extends the configuration options for patron.conf.  If the user
#  wishes to use the specific configuration settings the following needs to
#  be added to patron.conf:
#  [image_file_url]
...
#        with the glance deployment.  Both glance and patron-compute must be
...
#        may mount a shared file system on a different path than patron-compute.
...
#        file is copied.
#
#  If these values are not added to patron.conf and the file module is in the
#  allowed_direct_url_schemes list, then the legacy behavior will occur such
#  that a copy will be attempted assuming that the glance and patron file systems
#  are the same.
...
class FileTransfer(xfer_base.TransferBase):
    ...
    def _normalize_destination(self, patron_mount, glance_mount, path):
        if not path.startswith(glance_mount):
            ...
            raise exception.ImageDownloadModuleMetaDataError(
                module=str(self), reason=msg)
        new_path = path.replace(glance_mount, patron_mount, 1)
        return new_path
