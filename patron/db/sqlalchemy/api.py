...
from patron.i18n import _, _LI, _LE, _LW
...
def require_admin_context(f):
    ...
    def wrapper(*args, **kwargs):
        patron.context.require_admin_context(args[0])
...
def require_context(f):
    ...
    :py:func:`patron.context.authorize_project_context` and
    :py:func:`patron.context.authorize_user_context`.
    ...
    @functools.wraps(f)
    def wrapper(*args, **kwargs):
        patron.context.require_context(args[0])
        return f(*args, **kwargs)
    return wrapper
...
def model_query(context, model,
    ...
    if patron.context.is_user_context(context) and project_only:
...
def service_get_by_compute_host(context, host, use_slave=False):
    result = model_query(context, models.Service, read_deleted="no",
                         use_slave=use_slave).\
                filter_by(host=host).\
                filter_by(binary='patron-compute').\
                first()
...
def compute_node_statistics(context):
    ...
    result = model_query(context,
                         ...
                         filter(models.Service.binary == "patron-compute").\
                         filter(_filter).\
                         first()
...
def floating_ip_allocate_address(context, project_id, pool,
                                 auto_assigned=False):
    patron.context.authorize_project_context(context, project_id)
    session = get_session()
    with session.begin():
...
def _floating_ip_get_by_address(context, address, session=None):
    ...
    if result.project_id and patron.context.is_user_context(context):
        patron.context.authorize_project_context(context, result.project_id)
...
def fixed_ip_get(context, id, get_network=False):
    ...
    # results?
    if (patron.context.is_user_context(context) and
            result['instance_uuid'] is not None):
        instance = instance_get_by_uuid(context.elevated(read_deleted='yes'),
                                        result['instance_uuid'])
        patron.context.authorize_project_context(context, instance.project_id)
...
def _fixed_ip_get_by_address(context, address, session=None,
    ...
    with session.begin(subtransactions=True):
        ...
        if (patron.context.is_user_context(context) and
            ...
            patron.context.authorize_project_context(context,
                                                   instance.project_id)
...
def key_pair_destroy(context, user_id, name):
    patron.context.authorize_user_context(context, user_id)
...
def quota_class_get_all_by_name(context, class_name):
    patron.context.authorize_quota_class_context(context, class_name)
...
def archive_deleted_rows_for_table(context, tablename, max_rows):
    ...
    """
    # NOTE(guochbo): There is a circular import, patron.db.sqlalchemy.utils
    # imports patron.db.sqlalchemy.api.
    from patron.db.sqlalchemy import utils as db_utils
...
def migrate_flavor_data(context, max_count, flavor_cache):
    # NOTE(danms): This is only ever run in patron-manage, and we need to avoid
    # a circular import
