...
from patron.db.sqlalchemy import api as db
...
def create_shadow_table(migrate_engine, table_name=None, table=None,
    ...
    if table_name is None and table is None:
        raise exception.PatronException(_("Specify `table_name` or `table` "
                                        "param"))
    if not (table_name is None or table is None):
        raise exception.PatronException(_("Specify only one param `table_name` "
                                        "`table`"))
