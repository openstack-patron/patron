    ...
    if col_name in table.columns:
        ...
        if records:
            msg = _("There are %(records)d records in the "
                    ...
                    "'patron-manage db null_instance_uuid_scan' command.") % (
...
def process_null_records(meta, scan=True):
    ...
    if scan:
        for table in reversed(meta.sorted_tables):
            ...
            # that calls patron.db.api.fixed_ip_disassociate_all_by_timeout which
...
def upgrade(migrate_engine):
    ...
    # have to do this explicitly because the foreign keys in patron aren't
