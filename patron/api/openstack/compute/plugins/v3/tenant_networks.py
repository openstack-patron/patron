...
from patron.api.openstack.compute.schemas.v3 import tenant_networks as schema
...
class TenantNetworkController(wsgi.Controller):
    def __init__(self, network_api=None):
        self.network_api = patron.network.API(skip_policy_check=True)
...
def _sync_networks(context, project_id, session):
    ctx = patron_context.RequestContext(user_id=None, project_id=project_id)
    ctx = ctx.elevated()
    networks = patron.network.api.API().get_all(ctx)
