...
from patron.cells import messaging
...
class CellsManager(manager.Manager):
    """The patron-cells manager class.  This class defines RPC
    methods that the local cell may call.  This class is NOT used for
    messages coming from other cells.  That communication is
    driver-specific.

    Communication to other cells happens via the patron.cells.messaging module.
    ...
    def _heal_instances(self, ctxt):
        ...
        patron-cells services aren't attempting to sync the same instances
        in lockstep.
