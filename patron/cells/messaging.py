...
from patron.cells import state as cells_state
...
cell_messaging_opts = [
    ...
    cfg.StrOpt('scheduler',
            default='patron.cells.scheduler.CellsScheduler',
            help='Cells scheduler to use')]

CONF = cfg.CONF
CONF.import_opt('name', 'patron.cells.opts', group='cells')
CONF.import_opt('call_timeout', 'patron.cells.opts', group='cells')
...
class _BaseMessage(object):
    ...
    fanout means to send to all patron-cells services running in a cell.
    This is useful for capacity and capability broadcasting as well
    as making sure responses get back to the patron-cells service that
    is waiting.
    """
...
class _ResponseMessage(_TargetedMessage):
    ...
    original target message.  Because multiple patron-cells services may
...
class MessageRunner(object):
    ...
    def ask_children_for_capabilities(self, ctxt):
        ...
        called on startup of the patron-cells service.
        """
