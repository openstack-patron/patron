...
from patron.virt.disk.mount import api as mount
from patron.virt.disk.vfs import api as vfs
    ...
    if dev is None:
        LOG.error(_LE("Failed to mount container filesystem '%(image)s' "
                  ...
                  {"image": img, "target": container_dir,
                   "errors": img.errors})
        raise exception.PatronException(img.errors)
...
def _set_passwd(username, admin_passwd, passwd_data, shadow_data):
    ...
    :returns: nothing
    :raises: exception.PatronException(), IOError()

    """
    if os.name == 'nt':
        raise exception.PatronException(_('Not implemented on Windows'))
    ...
    for entry in p_file:
        ...
        raise exception.PatronException(msg % username)
