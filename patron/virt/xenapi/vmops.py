...
from patron.virt.xenapi import agent as xapi_agent
...
from patron.virt.xenapi import volumeops
...
xenapi_vmops_opts = [
    ...
    cfg.StrOpt('vif_driver',
               default='patron.virt.xenapi.vif.XenAPIBridgeDriver',
               help='The XenAPI VIF driver using XenServer Network APIs.'),
    cfg.StrOpt('image_upload_handler',
                default='patron.virt.xenapi.image.glance.GlanceStore',
...
class VMOps(object):
    ...
    def _get_agent(self, instance, vm_ref):
        if self.agent_enabled(instance):
            return xapi_agent.XenAPIBasedAgent(self._session, self._virtapi,
                                               instance, vm_ref)
        raise exception.PatronException(_("Error: Agent is disabled"))
    ...
    def list_instance_uuids(self):
        """Get the list of patron instance uuids for VMs found on the
        hypervisor.
        """
        patron_uuids = []
        for vm_ref, vm_rec in vm_utils.list_vms(self._session):
            other_config = vm_rec['other_config']
            patron_uuid = other_config.get('patron_uuid')
            if patron_uuid:
                patron_uuids.append(patron_uuid)
        return patron_uuids
    ...
    def get_all_bw_counters(self):
        ...
        for vm_ref, vm_rec in vm_utils.list_vms(self._session):
            ...
            if 'patron_uuid' not in vm_rec['other_config']:
                continue
    ...
    def check_can_live_migrate_destination(self, ctxt, instance_ref,
        ...
        :param instance_ref: patron.db.sqlalchemy.models.Instance object
    ...
    def get_per_instance_usage(self):
        ...
        def _get_uuid(vm_rec):
            other_config = vm_rec['other_config']
            return other_config.get('patron_uuid', None)
