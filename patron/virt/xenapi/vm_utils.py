...
from patron.virt.disk.vfs import localfs as vfsimpl
from patron.virt import hardware
from patron.virt import netutils
from patron.virt.xenapi import agent
from patron.virt.xenapi.image import utils as image_utils
...
xenapi_vm_utils_opts = [
    cfg.StrOpt('cache_images',
               ...
               help='Cache glance images locally. `all` will cache all'
                    ...
                    ' image_property `cache_in_patron=True`, and `none` turns'
...
def create_vm(session, instance, name_label, kernel, ramdisk,
    ...
    rec = {
        ...
        'other_config': {'patron_uuid': str(instance['uuid'])},
        'PCI_bus': '',
...
def create_vbd(session, vm_ref, vdi_ref, userdevice, vbd_type='disk',
    ...
    if osvol:
        ...
        # attached patron (or cinder) volume
...
def get_vdi_for_vm_safely(session, vm_ref, userdevice='0'):
    ...
    raise exception.PatronException(_("No primary VDI found for %s") % vm_ref)
...
def create_image(context, session, instance, name_label, image_id,
    ...
    if image_type == ImageType.DISK_ISO:
        ...
        try:
            cache = strutils.bool_from_string(sys_meta['image_cache_in_patron'])
        except KeyError:
            cache = False
...
def _default_download_handler():
    ...
    return importutils.import_object(
            'patron.virt.xenapi.image.glance.GlanceStore')


def _choose_download_handler(context, instance):
    if _image_uses_bittorrent(context, instance):
        return importutils.import_object(
                'patron.virt.xenapi.image.bittorrent.BittorrentStore')
    else:
...
def cleanup_attached_vdis(session):
    ...
    for vbd_ref in vbd_refs:
        ...
        if 'patron_instance_uuid' in vdi_rec['other_config']:
