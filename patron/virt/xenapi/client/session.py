...
from patron.virt.xenapi.client import objects as cli_objects
...
class XenAPISession(object):
    ...
    def __init__(self, url, user, pw):
        ...
        self.patron_version = _('%(vendor)s %(product)s %(version)s') % \
    ...
    def _verify_plugin_version(self):
        ...
        current_version = self.call_plugin_serialized(
            'patron_plugin_version', 'get_version')
    ...
    def _get_host_ref(self):
        """Return the xenapi host on which patron-compute runs on."""
        with self._get_session() as session:
