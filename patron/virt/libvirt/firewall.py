...
import patron.virt.firewall as base_firewall
...
class NWFilterFirewall(base_firewall.FirewallDriver):
    ...
    libvirt's nwfilter.
    all instances get a filter ("patron-base") applied. This filter
    ...
    def __init__(self, virtapi, host, **kwargs):
        ...
        :param virtapi: patron.virt.virtapi.VirtAPI instance
        :param host: patron.virt.libvirt.host.Host instance
    ...
    def patron_no_nd_reflection_filter(self):
        """This filter protects false positives on IPv6 Duplicate Address
        Detection(DAD).
        """
        uuid = self._get_filter_uuid('patron-no-nd-reflection')
        return '''<filter name='patron-no-nd-reflection' chain='ipv6'>
                  <!-- no nd reflection -->
    ...
    def patron_dhcp_filter(self):
        """The standard allow-dhcp-server filter is an <ip> one, so it uses
           ebtables to allow traffic through. Without a corresponding rule in
           iptables, it'll get blocked anyway.
        """
        uuid = self._get_filter_uuid('patron-allow-dhcp-server')
        return '''<filter name='patron-allow-dhcp-server' chain='ipv4'>
                    <uuid>%s</uuid>
    ...
    def _ensure_static_filters(self):
        ...
        self._define_filter(self.patron_no_nd_reflection_filter())
        filter_set.append('patron-no-nd-reflection')
        self._define_filter(self._filter_container('patron-nodhcp', filter_set))
        filter_set.append('allow-dhcp-server')
        self._define_filter(self._filter_container('patron-base', filter_set))
        self._define_filter(self._filter_container('patron-vpn',
                                                   ['allow-dhcp-server']))
        self._define_filter(self.patron_dhcp_filter())
    ...
    def _instance_filter_name(instance, nic_id=None):
        if not nic_id:
            return 'patron-instance-%s' % (instance.name)
        return 'patron-instance-%s-%s' % (instance.name, nic_id)

    def instance_filter_exists(self, instance, network_info):
        """Check patron-instance-instance-xxx exists."""
        for vif in network_info:
            nic_id = vif['address'].replace(':', '')
            instance_filter_name = self._instance_filter_name(instance, nic_id)
            try:
...
class IptablesFirewallDriver(base_firewall.IptablesFirewallDriver):
    def __init__(self, virtapi, execute=None, **kwargs):
        ...
        maps to an instance of the patron.virt.libvirt.host.Host
        class.
        """
