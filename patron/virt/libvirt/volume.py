...
from patron.storage import linuxscsi
from patron import utils
from patron.virt.libvirt import config as vconfig
from patron.virt.libvirt import quobyte
from patron.virt.libvirt import remotefs
    ...
    def get_config(self, connection_info, disk_info):
        ...
        if conf.source_protocol == 'iscsi':
            try:
                ...
                raise exception.PatronException(_("Invalid volume source data"))

            ip, port = utils.parse_server_string(target_portal)
            if ip == '' or port == '':
                raise exception.PatronException(_("Invalid target_lun"))
...
class LibvirtISCSIVolumeDriver(LibvirtBaseVolumeDriver):
    ...
    def connect_volume(self, connection_info, disk_info):
        ...
        # multipath should be configured on the patron-compute node,
        ...
        while ((self._get_transport() != "default" and not host_device)
          or not os.path.exists(host_device)):
            if tries >= self.num_scan_tries:
                raise exception.PatronException(_("iSCSI device not found at %s")
                                              % (host_device))
    ...
    def _connect_to_iscsi_portal(self, iscsi_properties):
        # NOTE(vish): If we are on the same host as patron volume, the
    ...
    def connect_volume(self, connection_info, mount_device):
        ...
        def _wait_for_device_discovery(aoedevpath, mount_device):
            ...
            if self.tries >= CONF.libvirt.num_aoe_discover_tries:
                raise exception.PatronException(_("AoE device not found at %s") %
                                                (aoedevpath))
