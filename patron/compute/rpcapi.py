...
def _compute_host(host, instance):
    ...
    if host:
        return host
    if not instance:
        raise exception.PatronException(_('No compute host specified'))
    if not instance.host:
        raise exception.PatronException(_('Unable to find host for '
    ...
    def _warn_buggy_live_migrations(self, data=None):
        ...
        if should_warn:
            LOG.warning(_LW('Live migration with clients before RPC version '
                            ...
                            'storage. See '
                            'https://bugs.launchpad.net/patron/+bug/1250751 for '
                            'more information!'))
