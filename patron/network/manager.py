...
from patron.network import driver
...
network_opts = [
    ...
    cfg.StrOpt('l3_lib',
               default='patron.network.l3.LinuxNetL3',
...
CONF.import_opt('network_topic', 'patron.network.rpcapi')
CONF.import_opt('fake_network', 'patron.network.linux_net')
CONF.import_opt('share_dhcp_address', 'patron.objects.network')
...
class NetworkManager(manager.Manager):
    ...
    def _get_subnets_from_network(self, context, network, instance_host=None):
        """Returns the 1 or 2 possible subnets for a patron network."""
        ...
        try:
            ...
            if teardown:
                ...
                if CONF.force_dhcp_release:
                    ...
                    #             get called by patron-dhcpbridge.
                    try:
    ...
    def create_networks(self, context,
        ...
        # Size of "label" column in patron.networks is 255, hence the restriction
        # NOTE: We can remove this check after v2.0 API code is removed because
        # jsonschema has checked already before this.
        if len(label) > 255:
    ...
    def _do_create_networks(self, context,
        ...
        for index, (subnet_v4, subnet_v6) in enumerate(subnets):
            ...
            if CONF.network_manager == 'patron.network.manager.VlanManager':
...
class FlatManager(NetworkManager):
    ...
    networks through patron-manage. This bridge needs to be created on all
    compute hosts.

    The idea is to create a single network for the host with a command like:
    patron-manage network create 192.168.0.0/24 1 256. Creating multiple
    ...
    Metadata forwarding must be handled by the gateway, and since patron does
...
class VlanManager(RPCAllocateFixedIP, floating_ips.FloatingIP, NetworkManager):
    ...
    and associated subnets are created with patron-manage using a command like:
    patron-manage network create 10.0.0.0/8 3 16.  This will create 3 networks
