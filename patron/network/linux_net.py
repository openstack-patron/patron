...
from patron.i18n import _, _LE, _LW
...
linux_net_opts = [
    cfg.MultiStrOpt('dhcpbridge_flagfile',
                    default=['/etc/patron/patron-dhcpbridge.conf'],
    ...
    cfg.StrOpt('dhcpbridge',
               default=paths.bindir_def('patron-dhcpbridge'),
               help='Location of patron-dhcpbridge'),
    ...
    cfg.StrOpt('linuxnet_interface_driver',
               default='patron.network.linux_net.LinuxBridgeInterfaceDriver',
...
CONF.import_opt('network_device_mtu', 'patron.objects.network')
...
class IptablesTable(object):
    ...
    def add_chain(self, name, wrap=True):
        ...
        so if patron-compute creates a chain named 'OUTPUT', it'll actually
        end up named 'patron-compute-OUTPUT'.
...
class IptablesManager(object):
    ...
    First, patron-filter-top. It's added at the top of FORWARD and OUTPUT. Its
    name is not wrapped, so it's shared between the various patron workers. It's
    ...
    "local" which is jumped to from patron-filter-top.
    ...
    def __init__(self, execute=None):
        ...
        # Add a patron-filter-top chain. It's intended to be shared
        # among the various patron components. It sits at the very top
        # of FORWARD and OUTPUT.
        for tables in [self.ipv4, self.ipv6]:
            tables['filter'].add_chain('patron-filter-top', wrap=False)
            tables['filter'].add_rule('FORWARD', '-j patron-filter-top',
                                      wrap=False, top=True)
            tables['filter'].add_rule('OUTPUT', '-j patron-filter-top',
                                      wrap=False, top=True)

            tables['filter'].add_chain('local')
            tables['filter'].add_rule('patron-filter-top', '-j $local',
                                      wrap=False)
        ...
        # Add a patron-postrouting-bottom chain. It's intended to be shared
        # among the various patron components. We set it as the last chain
        # of POSTROUTING chain.
        self.ipv4['nat'].add_chain('patron-postrouting-bottom', wrap=False)
        self.ipv4['nat'].add_rule('POSTROUTING', '-j patron-postrouting-bottom',
                                  wrap=False)

        # We add a snat chain to the shared patron-postrouting-bottom chain
        # so that it's applied last.
        self.ipv4['nat'].add_chain('snat')
        self.ipv4['nat'].add_rule('patron-postrouting-bottom', '-j $snat',
                                  wrap=False)
    ...
    def _modify_rules(self, current_lines, table, table_name):
        ...
        if not current_lines:
            fake_table = ['#Generated by patron',
                          '*' + table_name, 'COMMIT',
                          '#Completed by patron']
        ...
        def _weed_out_removes(line):
            ...
            if line.startswith(':'):
                # it's a chain, for example, ":patron-billing - [0:0]"
...
def _dhcp_file(dev, kind):
    ...
    return os.path.abspath('%s/patron-%s.%s' % (CONF.networks_path,
                                              dev,
                                              kind))
...
def _ra_file(dev, kind):
    ...
    fileutils.ensure_tree(CONF.networks_path)
    return os.path.abspath('%s/patron-ra-%s.%s' % (CONF.networks_path,
                                              dev,
                                              kind))
