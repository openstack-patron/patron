...
class SecurityGroupAPI(security_group_base.SecurityGroupBase):
    ...
    def _convert_to_patron_security_group_format(self, security_group):
        patron_group = {}
        patron_group['id'] = security_group['id']
        patron_group['description'] = security_group['description']
        patron_group['name'] = security_group['name']
        patron_group['project_id'] = security_group['tenant_id']
        patron_group['rules'] = []
        for rule in security_group.get('security_group_rules', []):
            if rule['direction'] == 'ingress':
                patron_group['rules'].append(
                    self._convert_to_patron_security_group_rule_format(rule))

        return patron_group

    def _convert_to_patron_security_group_rule_format(self, rule):
        patron_rule = {}
        patron_rule['id'] = rule['id']
        patron_rule['parent_group_id'] = rule['security_group_id']
        patron_rule['protocol'] = rule['protocol']
        if (patron_rule['protocol'] and rule.get('port_range_min') is None and
                rule.get('port_range_max') is None):
            if rule['protocol'].upper() in ['TCP', 'UDP']:
                patron_rule['from_port'] = 1
                patron_rule['to_port'] = 65535
            else:
                patron_rule['from_port'] = -1
                patron_rule['to_port'] = -1
        else:
            patron_rule['from_port'] = rule.get('port_range_min')
            patron_rule['to_port'] = rule.get('port_range_max')
        patron_rule['group_id'] = rule['remote_group_id']
        patron_rule['cidr'] = self.parse_cidr(rule.get('remote_ip_prefix'))
        return patron_rule
    ...
    def _make_neutron_security_group_rules_list(self, rules):
        ...
        for rule in rules:
            new_rule = {}
            # patron only supports ingress rules so all rules are ingress.
            ...
            # patron api wants to remove the rule we'd have to have some way to
