...
from patron.hacking import checks
from patron import test


class HackingTestCase(test.NoDBTestCase):
    """This class tests the hacking checks in patron.hacking.checks by passing
    ...
    def test_virt_driver_imports(self):
        ...
        self.assertEqual(expect, checks.import_no_virt_driver_import_deps(
            ...
            "./patron/virt/xenapi/driver.py"))

        self.assertEqual(expect, checks.import_no_virt_driver_import_deps(
            "import patron.virt.libvirt.utils as libvirt_utils",
            "./patron/virt/xenapi/driver.py"))

        self.assertIsNone(checks.import_no_virt_driver_import_deps(
            "from patron.virt.libvirt import utils as libvirt_utils",
            "./patron/virt/libvirt/driver.py"))

        self.assertIsNone(checks.import_no_virt_driver_import_deps(
            "import patron.virt.firewall",
            "./patron/virt/libvirt/firewall.py"))
    ...
    def test_virt_driver_config_vars(self):
        self.assertIsInstance(checks.import_no_virt_driver_config_deps(
            ...
            "'patron.virt.libvirt.driver', group='libvirt')",
            "./patron/virt/xenapi/driver.py"), tuple)

        self.assertIsNone(checks.import_no_virt_driver_config_deps(
            "CONF.import_opt('volume_drivers', "
            "'patron.virt.libvirt.driver', group='libvirt')",
            "./patron/virt/libvirt/volume.py"))
    ...
    def test_no_translate_debug_logs(self):
        self.assertEqual(len(list(checks.no_translate_debug_logs(
            "LOG.debug(_('foo'))", "patron/scheduler/foo.py"))), 1)

        self.assertEqual(len(list(checks.no_translate_debug_logs(
            "LOG.debug('foo')", "patron/scheduler/foo.py"))), 0)

        self.assertEqual(len(list(checks.no_translate_debug_logs(
            "LOG.info(_('foo'))", "patron/scheduler/foo.py"))), 0)

    def test_no_setting_conf_directly_in_tests(self):
        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option = 1", "patron/tests/test_foo.py"))), 1)

        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.group.option = 1", "patron/tests/test_foo.py"))), 1)

        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option = foo = 1", "patron/tests/test_foo.py"))), 1)

        # Shouldn't fail with comparisons
        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option == 'foo'", "patron/tests/test_foo.py"))), 0)

        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option != 1", "patron/tests/test_foo.py"))), 0)

        # Shouldn't fail since not in patron/tests/
        self.assertEqual(len(list(checks.no_setting_conf_directly_in_tests(
            "CONF.option = 1", "patron/compute/foo.py"))), 0)
