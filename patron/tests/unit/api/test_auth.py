...
class TestNovaKeystoneContextMiddleware(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        @webob.dec.wsgify()
        def fake_app(req):
            self.context = req.environ['patron.context']
            return webob.Response()

        self.context = None
        self.middleware = patron.api.auth.PatronKeystoneContext(fake_app)
...
class TestKeystoneMiddlewareRoles(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.middleware = patron.api.auth.PatronKeystoneContext(role_check_app)
        self.request = webob.Request.blank('/')
        self.request.headers['X_USER'] = 'testuser'
        self.request.headers['X_TENANT_ID'] = 'testtenantid'
        self.request.headers['X_AUTH_TOKEN'] = 'testauthtoken'
...
class TestPipeLineFactory(test.NoDBTestCase):
    ...
    def test_pipeline_factory_noauthold(self):
        ...
        app = patron.api.auth.pipeline_factory(
    ...
    def test_pipeline_factory_v21_noauthold(self):
        fake_pipeline = 'test1 test2 test3'
        CONF.set_override('auth_strategy', 'noauth')
        app = patron.api.auth.pipeline_factory_v21(
            TestPipeLineFactory.FakeLoader(), None, noauth=fake_pipeline)
        self._test_pipeline(fake_pipeline, app)
