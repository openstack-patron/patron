...
from patron.api.openstack.compute.contrib import os_tenant_networks as networks
from patron.api.openstack.compute.plugins.v3 import tenant_networks \
     as networks_v21
...
class TenantNetworksTestV21(test.NoDBTestCase):
    ...
    @mock.patch('patron.network.api.API.delete',
    ...
    @mock.patch('patron.quota.QUOTAS.rollback')
    @mock.patch('patron.network.api.API.delete')
    ...
    @mock.patch('patron.network.api.API.get')
    ...
    @mock.patch('patron.network.api.API.get_all')
    def _test_network_index(self, get_all_mock, default_net=True):
        ...
        if default_net is True:
            self.req.environ['patron.context'].project_id = DEFAULT_TENANT_ID
    ...
    @mock.patch('patron.quota.QUOTAS.reserve')
    @mock.patch('patron.quota.QUOTAS.commit')
    @mock.patch('patron.network.api.API.create')
