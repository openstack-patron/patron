...
import iso8601
from patron.api.openstack.compute.contrib import availability_zone as az_v2
from patron.api.openstack.compute import plugins
from patron.api.openstack.compute.plugins.v3 import availability_zone as az_v21
from patron.api.openstack.compute.plugins.v3 import servers as servers_v21
from patron.api.openstack.compute import servers as servers_v2
...
def fake_service_get_all(context, disabled=None):
    ...
    if disabled:
        return [__fake_service("patron-compute", "zone-2",
                               datetime.datetime(2012, 11, 14, 9, 53, 25, 0),
                               datetime.datetime(2012, 12, 26, 14, 45, 25, 0),
                               "fake_host-1", True),
                __fake_service("patron-scheduler", "internal",
                               datetime.datetime(2012, 11, 14, 9, 57, 3, 0),
                               datetime.datetime(2012, 12, 26, 14, 45, 25, 0),
                               "fake_host-1", True),
                __fake_service("patron-network", "internal",
                               datetime.datetime(2012, 11, 16, 7, 25, 46, 0),
                               datetime.datetime(2012, 12, 26, 14, 45, 24, 0),
                               "fake_host-2", True)]
    else:
        return [__fake_service("patron-compute", "zone-1",
                               datetime.datetime(2012, 11, 14, 9, 53, 25, 0),
                               datetime.datetime(2012, 12, 26, 14, 45, 25, 0),
                               "fake_host-1", False),
                __fake_service("patron-sched", "internal",
                               datetime.datetime(2012, 11, 14, 9, 57, 3, 0),
                               datetime.datetime(2012, 12, 26, 14, 45, 25, 0),
                               "fake_host-1", False),
                __fake_service("patron-network", "internal",
...
def fake_service_is_up(self, service):
    return service['binary'] != u"patron-network"
...
class AvailabilityZoneApiTestV21(test.NoDBTestCase):
    ...
    def test_availability_zone_detail(self):
        ...
        patron_network_timestamp = iso8601.parse_date("2012-12-26T14:45:24Z")
        expected = [{'zoneName': 'zone-1',
                    'zoneState': {'available': True},
                    'hosts': {'fake_host-1': {
                        'patron-compute': {'active': True, 'available': True,
                                         'updated_at': timestamp}}}},
                   {'zoneName': 'internal',
                    'zoneState': {'available': True},
                    'hosts': {'fake_host-1': {
                        'patron-sched': {'active': True, 'available': True,
                                       'updated_at': timestamp}},
                              'fake_host-2': {
                                  'patron-network': {
                                      'active': True,
                                      'available': False,
                                      'updated_at': patron_network_timestamp}}}},
                   {'zoneName': 'zone-2',
                    'zoneState': {'available': False},
                    'hosts': None}]
