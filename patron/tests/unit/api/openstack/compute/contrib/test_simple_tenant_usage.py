...
from patron.api.openstack.compute.contrib import simple_tenant_usage as \
    simple_tenant_usage_v2
from patron.api.openstack.compute.plugins.v3 import simple_tenant_usage as \
...
class SimpleTenantUsageTestV21(test.TestCase):
    ...
    def _test_verify_index(self, start, stop):
        ...
        req.environ['patron.context'] = self.admin_context
    ...
    def _test_verify_show(self, start, stop):
        ...
        req = fakes.HTTPRequest.blank('?start=%s&end=%s' %
                    (start.isoformat(), stop.isoformat()))
        req.environ['patron.context'] = self.user_context
