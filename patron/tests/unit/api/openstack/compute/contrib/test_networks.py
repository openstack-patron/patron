...
from patron.api.openstack.compute.contrib import networks_associate
from patron.api.openstack.compute.contrib import os_networks as networks
from patron.api.openstack.compute.plugins.v3 import networks as networks_v21
from patron.api.openstack.compute.plugins.v3 import networks_associate as \
...
from patron.network.neutronv2 import api as neutron
        ...
        for network in self.networks:
            if network.get('uuid') == network_id:
                if 'injected' in network and network['injected'] is None:
                    ...
                    # When using patron-network, 'injected' value should be
...
class NetworksTestV21(test.NoDBTestCase):
    ...
    def test_network_list_all_as_user(self):
        ...
        project_id = self.req.environ["patron.context"].project_id
        cxt = self.req.environ["patron.context"]
    ...
    @mock.patch('patron.tests.unit.api.openstack.compute.contrib.test_networks.'
