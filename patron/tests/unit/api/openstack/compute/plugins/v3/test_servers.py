...
from patron.api.openstack import compute
from patron.api.openstack.compute import plugins
from patron.api.openstack.compute.plugins.v3 import disk_config
from patron.api.openstack.compute.plugins.v3 import ips
from patron.api.openstack.compute.plugins.v3 import keypairs
from patron.api.openstack.compute.plugins.v3 import servers
from patron.api.openstack.compute.schemas.v3 import disk_config as \
    disk_config_schema
from patron.api.openstack.compute.schemas.v3 import servers as servers_schema
from patron.api.openstack.compute import views
...
from patron.network import manager
from patron.network.neutronv2 import api as neutron_api
...
from patron import utils as patron_utils

CONF = cfg.CONF
CONF.import_opt('password_length', 'patron.utils')
...
class ServersControllerTest(ControllerTest):
    ...
    def test_requested_networks_neutronv2_enabled_with_port(self):
        self.flags(network_api_class='patron.network.neutronv2.api.API')
    ...
    def test_requested_networks_with_duplicate_networks(self):
        # duplicate networks are allowed only for patron neutron v2.0
        network = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'
    ...
    def test_requested_networks_neutronv2_subclass_with_port(self):
        cls = ('patron.tests.unit.api.openstack.compute' +
    ...
    def test_create_instance_above_quota_server_group_members(self):
        ...
        def fake_count(context, name, group, user_id):
            ...
            self.assertEqual(user_id,
                             self.req.environ['patron.context'].user_id)
            return 10
...
class ServersViewBuilderTest(test.TestCase):
    ...
    def test_build_server_detail_with_fault_admin(self):
        ...
        self.request.environ['patron.context'].is_admin = True
        output = self.view_builder.show(self.request, self.instance)
        self.assertThat(output['server']['fault'],
                        matchers.DictMatches(expected_fault))
    ...
    def test_build_server_detail_with_metadata(self):
        ...
        metadata = patron_utils.metadata_to_dict(metadata)
...
class ServersAllExtensionsTestCase(test.TestCase):
    ...
    def test_update_missing_server(self):
        ...
        req.body = jsonutils.dumps(body)
        with mock.patch('patron.objects.Instance.save') as mock_save:
            res = req.get_response(self.app)
