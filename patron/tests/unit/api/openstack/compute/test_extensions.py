...
from patron.api.openstack.compute import extensions as compute_extensions
from patron.api.openstack import extensions as base_extensions
...
class ExtensionTestCase(test.TestCase):
    def setUp(self):
        ...
        fox = ('patron.tests.unit.api.openstack.compute.extensions.'
               'foxinsocks.Foxinsocks')
        if fox not in ext_list:
            ext_list.append(fox)
            self.flags(osapi_compute_extension=ext_list)
        self.fake_context = patron.context.RequestContext('fake', 'fake')
    ...
    def _test_extension_authorizer_throws_exception_if_policy_fails(self,
        ...
        patron.policy.enforce(self.fake_context,
                            rule,
                            target).AndRaise(
