...
class TestLoaderNothingExists(test.NoDBTestCase):
    ...
    def test_relpath_config_not_found(self):
        ...
        self.assertRaises(
            patron.exception.ConfigNotFound,
            patron.wsgi.Loader,
        )

    def test_asbpath_config_not_found(self):
        self.flags(api_paste_config='/etc/patron/api-paste.ini')
        self.assertRaises(
            patron.exception.ConfigNotFound,
            patron.wsgi.Loader,
        )
...
document_root = /tmp
    ...
    def setUp(self):
        ...
        self.config.seek(0)
        self.config.flush()
        self.loader = patron.wsgi.Loader(self.config.name)
    ...
    def test_app_not_found(self):
        self.assertRaises(
            patron.exception.PasteAppNotFound,
...
class TestWSGIServer(test.NoDBTestCase):
    ...
    def test_custom_max_header_line(self):
        ...
        patron.wsgi.Server("test_custom_max_header_line", None)
        self.assertEqual(CONF.max_header_line, eventlet.wsgi.MAX_HEADER_LINE)

    def test_start_random_port(self):
        server = patron.wsgi.Server("test_random_port", None,
    ...
    def test_socket_options_for_simple_server(self):
        ...
        server = patron.wsgi.Server("test_socket_options", None,
                                  host="127.0.0.1", port=0)
        server.start()
        sock = server._socket
    ...
    def test_server_pool_waitall(self):
        ...
        server = patron.wsgi.Server("test_server", None,
            host="127.0.0.1")
        server.start()
    ...
    def test_uri_length_limit(self):
        server = patron.wsgi.Server("test_uri_length_limit", None,
    ...
    def test_reset_pool_size_to_default(self):
        server = patron.wsgi.Server("test_resize", None,
            host="127.0.0.1", max_url_len=16384)
        server.start()
...
class TestWSGIServerWithSSL(test.NoDBTestCase):
    ...
    def test_ssl_server(self):
        ...
        fake_ssl_server = patron.wsgi.Server("fake_ssl", test_app,
                                           host="127.0.0.1", port=0,
                                           use_ssl=True)
    ...
    def test_two_servers(self):
        ...
        fake_server = patron.wsgi.Server("fake", test_app,
            host="127.0.0.1", port=0)
        fake_server.start()
    ...
    def test_app_using_ipv6_and_ssl(self):
        ...
        server = patron.wsgi.Server("fake_ssl",
                                  hello_world,
                                  host="::1",
                                  port=0,
                                  use_ssl=True)
