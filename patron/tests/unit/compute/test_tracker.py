...
from patron.compute import claims
...
def setup_rt(hostname, nodename, virt_resources=_VIRT_DRIVER_AVAIL_RESOURCES,
    ...
    with contextlib.nested(
            mock.patch('patron.conductor.API', return_value=cond_api_mock),
            mock.patch('patron.scheduler.client.SchedulerClient',
                       return_value=sched_client_mock),
            mock.patch('patron.rpc.get_notifier', return_value=notifier_mock)):
...
class TestUpdateAvailableResources(BaseTestCase):
    ...
    @mock.patch('patron.objects.Service.get_by_compute_host')
    @mock.patch('patron.objects.ComputeNode.get_by_host_and_nodename')
    @mock.patch('patron.objects.MigrationList.get_in_progress_by_host_and_node')
    @mock.patch('patron.objects.InstanceList.get_by_host_and_node')
...
class TestResizeClaim(BaseTestCase):
    def setUp(self):
        ...
        with contextlib.nested(
            mock.patch('patron.objects.InstanceList.get_by_host_and_node'),
            mock.patch('patron.objects.MigrationList.'
    ...
    @mock.patch('patron.objects.Flavor.get_by_id')
