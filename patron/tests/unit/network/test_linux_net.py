...
class LinuxNetworkUtilsTestCase(test.NoDBTestCase):
    def test_is_pid_cmdline_correct(self):
        ...
        with mock.patch.object(linux_net, 'open', fake_open, create=True):
            self.assertFalse(
                linux_net.is_pid_cmdline_correct(1, "patron-br100.conf"),
                "patron-br100.conf should not have been found")

        # Positive matching case
        fake_open = mock.mock_open(
            read_data=('/usr/sbin/dnsmasq '
                       '--dhcp-hostsfile='
                       '/opt/stack/data/patron/networks/patron-br100.conf'))
        with mock.patch.object(linux_net, 'open', fake_open, create=True):
            self.assertTrue(
                linux_net.is_pid_cmdline_correct(1, "patron-br100.conf"),
                'patron-br100.conf should have been found')
...
class LinuxNetworkTestCase(test.NoDBTestCase):
    ...
    def test_get_dhcp_hosts_for_nw00(self):
        ...
        expected = (
                "DE:AD:BE:EF:00:00,fake_instance00.patronlocal,"
                "192.168.0.100,net:NW-0\n"
                "DE:AD:BE:EF:00:03,fake_instance01.patronlocal,"
                "192.168.1.101,net:NW-3\n"
                "DE:AD:BE:EF:00:04,fake_instance00.patronlocal,"
    ...
    def test_get_dhcp_hosts_for_nw01(self):
        ...
        expected = (
                "DE:AD:BE:EF:00:02,fake_instance01.patronlocal,"
                "192.168.0.101,net:NW-2\n"
                "DE:AD:BE:EF:00:05,fake_instance01.patronlocal,"
    ...
    def test_get_dns_hosts_for_nw00(self):
        expected = (
                "192.168.0.100\tfake_instance00.patronlocal\n"
                "192.168.1.101\tfake_instance01.patronlocal\n"
                "192.168.0.102\tfake_instance00.patronlocal"
        )
    ...
    def test_get_dns_hosts_for_nw01(self):
        expected = (
                "192.168.1.100\tfake_instance00.patronlocal\n"
                "192.168.0.101\tfake_instance01.patronlocal\n"
                "192.168.1.102\tfake_instance01.patronlocal"
        )
    ...
    def test_host_dhcp_truncated_hostname(self):
        expected = ','.join(['DE:AD:BE:EF:00:07',
                             ...
                             'truncation_when_too_long.patronlocal',
                             '192.168.2.100'])
        fixedip = objects.FixedIPList.get_by_network(self.context,
                                                     {'id': 2})[0]
        actual = self.driver._host_dhcp(fixedip)
    ...
    def test_host_dns_without_default_gateway_network(self):
        expected = "192.168.0.100\tfake_instance00.patronlocal"
        fixedip = objects.FixedIPList.get_by_network(self.context,
                                                     {'id': 0})[0]
