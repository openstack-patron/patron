...
from patron.network.neutronv2 import api as neutronapi
from patron.network.neutronv2 import constants
from patron import objects
from patron.openstack.common import policy as common_policy
from patron.pci import manager as pci_manager
from patron.pci import whitelist as pci_whitelist
...
class TestNeutronClient(test.NoDBTestCase):
    ...
    @mock.patch('patron.network.neutronv2.api._ADMIN_AUTH')
...
class TestNeutronv2Base(test.TestCase):
    ...
    def setUp(self):
        ...
        self.fip_pool_patron = {'id': '435e20c3-d9f1-4f1b-bee5-4611a1dd07db',
                              'name': 'patron',
                              'router:external': True,
                              'tenant_id': 'admin_tenantid'}
...
class TestNeutronv2(TestNeutronv2Base):
    ...
    def test_get_instance_nw_info_remove_ports_from_neutron(self):
        ...
        # is also removed from the patron.
    ...
    def _fake_get_instance_nw_info_helper(self, network_cache,
        ...
        :param network_cache - data already in the patron network cache.
    ...
    @mock.patch('patron.network.neutronv2.api.API._unbind_ports')
    ...
    @mock.patch('patron.network.neutronv2.api.API._get_preexisting_port_ids')
    @mock.patch('patron.network.neutronv2.api.API._unbind_ports')
    ...
    def test_validate_networks_duplicate_disable(self):
        ...
        network ids are passed to validate_networks, when patron config flag
    ...
    def test_validate_networks_ports_in_same_network_disable(self):
        ...
        duplicate network are passed to validate_networks, when patron config
    ...
    def test_validate_networks_ports_in_same_network_enable(self):
        ...
        on same duplicate network are passed to validate_networks, when patron
    ...
    def test_validate_networks_with_ports_and_networks(self):
        # Test validation for a request for one instance needing
        # one port allocated via patron with another port being passed in.
        port_b = self.port_data2[1]
    ...
    def test_get_floating_ip_pools(self):
        ...
        self.moxed_client.list_networks(**search_opts).\
            AndReturn({'networks': [self.fip_pool, self.fip_pool_patron]})
        self.mox.ReplayAll()
        pools = api.get_floating_ip_pools(self.context)
        expected = [self.fip_pool['name'], self.fip_pool_patron['name']]
    ...
    def test_allocate_floating_ip_with_default_pool(self):
        api = neutronapi.API()
        pool_name = self.fip_pool_patron['name']
        pool_id = self.fip_pool_patron['id']
        search_opts = {'router:external': True,
                       'fields': 'id',
                       'name': pool_name}
        self.moxed_client.list_networks(**search_opts).\
            AndReturn({'networks': [self.fip_pool_patron]})
        self.moxed_client.create_floatingip(
            {'floatingip': {'floating_network_id': pool_id}}).\
            AndReturn({'floatingip': self.fip_unassociated})
        self.mox.ReplayAll()
    ...
    def test_associate_floating_ip(self):
        ...
        search_opts = {'device_owner': 'compute:patron',
    ...
    @mock.patch('patron.objects.Instance.get_by_uuid')
...
class TestNeutronv2WithMock(test.TestCase):
    ...
    @mock.patch('patron.network.neutronv2.api.API._get_preexisting_port_ids')
    @mock.patch('patron.network.neutronv2.api.API.'
    ...
    @mock.patch.object(neutronapi, 'get_client', return_value=mock.Mock())
    def _test_show_port_exceptions(self, client_exc, expected_patron_exc,
                                   get_client_mock):
        show_port_mock = mock.Mock(side_effect=client_exc)
        get_client_mock.return_value.show_port = show_port_mock
        self.assertRaises(expected_patron_exc, self.api.show_port,
    ...
    @mock.patch('patron.network.neutronv2.api.compute_utils')
    ...
    @mock.patch('patron.network.neutronv2.api.API._has_port_binding_extension')
    @mock.patch('patron.network.neutronv2.api.get_client')
    ...
    @mock.patch('patron.network.neutronv2.api.API.get_instance_nw_info')
    @mock.patch('patron.network.neutronv2.api.excutils')
    @mock.patch('patron.network.neutronv2.api.API._delete_ports')
    @mock.patch('patron.network.neutronv2.api.API.'
                '_check_external_network_attach')
    @mock.patch('patron.network.neutronv2.api.LOG')
    @mock.patch('patron.network.neutronv2.api.API._unbind_ports')
    @mock.patch('patron.network.neutronv2.api.API._has_port_binding_extension')
    @mock.patch('patron.network.neutronv2.api.API.'
                '_populate_neutron_extension_values')
    @mock.patch('patron.network.neutronv2.api.API._get_available_networks')
    @mock.patch('patron.network.neutronv2.api.get_client')
    ...
    @mock.patch('patron.objects.network_request.utils')
    @mock.patch('patron.network.neutronv2.api.LOG')
    @mock.patch('patron.network.neutronv2.api.base_api')
    @mock.patch('patron.network.neutronv2.api.API._delete_ports')
    @mock.patch('patron.network.neutronv2.api.API._unbind_ports')
    @mock.patch('patron.network.neutronv2.api.API._get_preexisting_port_ids')
    @mock.patch('patron.network.neutronv2.api.get_client')
