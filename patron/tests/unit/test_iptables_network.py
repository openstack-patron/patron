...
class IptablesManagerTestCase(test.NoDBTestCase):
    ...
    sample_filter = ['#Generated by iptables-save on Fri Feb 18 15:17:05 2011',
                     ...
                     ':patron-filter-top - [0:0]',
                     '[0:0] -A FORWARD -j patron-filter-top',
                     '[0:0] -A OUTPUT -j patron-filter-top',
                     '[0:0] -A patron-filter-top -j %s-local' % (binary_name),
    ...
    def test_nat_rules(self):
        ...
        self.assertIn('-j patron-postrouting-bottom', last_postrouting_line,
                        "Last POSTROUTING rule does not jump to "
                        "patron-postouting-bottom: %s" % last_postrouting_line)
        ...
        for chain in ['FORWARD', 'OUTPUT']:
            for line in new_lines:
                if line.startswith('[0:0] -A %s' % chain):
                    self.assertIn('-j patron-filter-top', line,
                                  "First %s rule does not "
                                  "jump to patron-filter-top" % chain)
                    break

        self.assertTrue('[0:0] -A patron-filter-top '
                        '-j %s-local' % self.binary_name in new_lines,
                        "patron-filter-top does not jump to wrapped local chain")
    ...
    def test_missing_table(self):
        ...
        self.assertTrue("#Generated by patron" == new_lines[0] and
                        "*filter" == new_lines[1] and
                        "COMMIT" == new_lines[-2] and
                        "#Completed by patron" == new_lines[-1],
