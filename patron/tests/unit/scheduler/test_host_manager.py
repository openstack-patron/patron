...
from patron.pci import stats as pci_stats
from patron.scheduler import filters
from patron.scheduler import host_manager
from patron.scheduler import utils as sched_utils
...
class HostManagerTestCase(test.NoDBTestCase):
    ...
    @mock.patch.object(patron.objects.InstanceList, 'get_by_filters')
    @mock.patch.object(patron.objects.ComputeNodeList, 'get_all')
    ...
    def test_get_all_host_states(self, mock_get_by_host):
        ...
        objects.ServiceList.get_by_binary(
            context, 'patron-compute').AndReturn(fakes.SERVICES)
    ...
    @mock.patch('patron.objects.ServiceList.get_by_binary')
    @mock.patch('patron.objects.ComputeNodeList.get_all')
    @mock.patch('patron.objects.InstanceList.get_by_host')
...
class HostStateTestCase(test.NoDBTestCase):
    ...
    @mock.patch('patron.virt.hardware.get_host_numa_usage_from_instance')
    @mock.patch('patron.virt.hardware.numa_fit_instance_to_host')
    @mock.patch('patron.virt.hardware.instance_topology_from_instance')
    @mock.patch('patron.virt.hardware.host_topology_and_format_from_host')
