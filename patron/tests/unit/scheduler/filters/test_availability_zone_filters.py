...
from patron.scheduler.filters import availability_zone_filter
from patron import test
from patron.tests.unit.scheduler import fakes


@mock.patch('patron.scheduler.filters.utils.aggregate_metadata_get_by_host')
class TestAvailabilityZoneFilter(test.NoDBTestCase):
    ...
    def test_availability_zone_filter_same(self, agg_mock):
        agg_mock.return_value = {'availability_zone': 'patron'}
        request = self._make_zone_request('patron')
        host = fakes.FakeHostState('host1', 'node1', {})
        self.assertTrue(self.filt_cls.host_passes(host, request))

    def test_availability_zone_filter_same_comma(self, agg_mock):
        agg_mock.return_value = {'availability_zone': 'patron,patron2'}
        request = self._make_zone_request('patron')
        host = fakes.FakeHostState('host1', 'node1', {})
        self.assertTrue(self.filt_cls.host_passes(host, request))
