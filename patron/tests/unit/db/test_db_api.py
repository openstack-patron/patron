...
from patron.db.sqlalchemy import api as sqlalchemy_api
from patron.db.sqlalchemy import models
from patron.db.sqlalchemy import types as col_types
...
CONF = cfg.CONF
CONF.import_opt('reserved_host_memory_mb', 'patron.compute.resource_tracker')
CONF.import_opt('reserved_host_disk_mb', 'patron.compute.resource_tracker')
...
class SqlAlchemyDbApiTestCase(DbTestCase):
    ...
    @mock.patch('patron.db.sqlalchemy.api.instance_get_all_by_filters_sort')
...
class InstanceTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    @mock.patch('patron.db.sqlalchemy.api.undefer')
    @mock.patch('patron.db.sqlalchemy.api.joinedload')
    ...
    @mock.patch('patron.db.sqlalchemy.api._instances_fill_metadata')
    @mock.patch('patron.db.sqlalchemy.api._instance_get_all_query')
    ...
    def test_instance_update_and_get_original_no_conflict_on_session(self):
        ...
        with mock.patch("patron.db.sqlalchemy.api.get_session", lambda: session):
            instance = self.create_instance_with_args()
            (old_ref, new_ref) = db.instance_update_and_get_original(
                self.ctxt, instance['uuid'], {'metadata': {'mk1': 'mv3'}})
...
class ServiceTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def test_service_get_by_compute_host(self):
        values = [
            {'host': 'host1', 'binary': 'patron-compute'},
            {'host': 'host2', 'binary': 'patron-scheduler'},
            {'host': 'host3', 'binary': 'patron-compute'}
        ]
        services = [self._create_service(vals) for vals in values]
...
class ComputeNodeTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def setUp(self):
        ...
        self.ctxt = context.get_admin_context()
        self.service_dict = dict(host='host1', binary='patron-compute',
    ...
    def test_compute_node_statistics_with_other_service(self):
        ...
        other_service['binary'] = 'patron-fake'
...
class FlavorMigrationTestCase(test.TestCase):
    ...
    @mock.patch('patron.db.sqlalchemy.api._augment_flavor_to_migrate')
    ...
    @mock.patch('patron.db.sqlalchemy.api._augment_flavor_to_migrate')
    @mock.patch('patron.db.sqlalchemy.api.flavor_get_by_flavor_id')
