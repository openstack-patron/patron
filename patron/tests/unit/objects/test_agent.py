...
from patron.objects import agent as agent_obj
...
fake_agent = {
    ...
    'architecture': 'DISC',
    'version': '1.0',
    'url': 'http://openstack.org/patronvm/agents/patronvm_agent_v1.0.rpm',
...
class _TestAgent(object):
    ...
    @mock.patch('patron.db.agent_build_get_all')
    def test_get_all(self, mock_get_all):
        mock_get_all.return_value = [fake_agent]
        agents = agent_obj.AgentList.get_all(self.context, hypervisor='patronvm')
        self.assertEqual(1, len(agents))
        self._compare(self, fake_agent, agents[0])
        mock_get_all.assert_called_once_with(self.context, hypervisor='patronvm')
