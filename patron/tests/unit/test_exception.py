...
class PatronExceptionTestCase(test.NoDBTestCase):
    def test_default_error_msg(self):
        class FakePatronException(exception.PatronException):
            msg_fmt = "default message"

        exc = FakePatronException()
        self.assertEqual(six.text_type(exc), 'default message')

    def test_error_msg(self):
        self.assertEqual(six.text_type(exception.PatronException('test')),
                         'test')
    ...
    def test_default_error_msg_with_kwargs(self):
        ...
        exc = FakePatronException(code=500)
    ...
    def test_error_msg_exception_with_kwargs(self):
        ...
        exc = FakePatronException(code=500, misspelled_code='blah')
    ...
    def test_error_code_from_kwarg(self):
        ...
        exc = FakePatronException(code=404)
        self.assertEqual(exc.kwargs['code'], 404)
    ...
    def test_format_message_remote(self):
        class FakePatronException_Remote(exception.PatronException):
            msg_fmt = "some message"

            def __unicode__(self):
                return u"print the whole trace"

        exc = FakePatronException_Remote()
    ...
    def test_format_message_remote_error(self):
        ...
        exc = FakePatronException_Remote(lame_arg='lame')
...
class ExceptionValidMessageTestCase(test.NoDBTestCase):
    ...
    def test_messages(self):
        ...
        for name, obj in inspect.getmembers(exception):
            if name in ['PatronException', 'InstanceFaultRollback']:
                continue

            if not inspect.isclass(obj):
                continue

            if not issubclass(obj, exception.PatronException):
                continue
