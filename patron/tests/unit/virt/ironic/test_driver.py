...
from patron.compute import power_state as patron_states
...
from patron.tests.unit.virt.ironic import utils as ironic_utils
...
from patron.virt.ironic import client_wrapper as cw
from patron.virt.ironic import driver as ironic_driver
from patron.virt.ironic import ironic_states
...
class IronicDriverTestCase(test.NoDBTestCase):
    ...
    def test_get_info(self, mock_gbiu):
        ...
        self.assertEqual(hardware.InstanceInfo(state=patron_states.RUNNING,
    ...
    def test_get_info_http_not_found(self, mock_gbiu):
        ...
        result = self.driver.get_info(instance)
        self.assertEqual(hardware.InstanceInfo(state=patron_states.NOSTATE),
                         result)
    ...
    def test_spawn_node_trigger_deploy_fail(self, mock_cleanup_deploy,
        ...
        mock_node.set_provision_state.side_effect = exception.PatronException()
        self.assertRaises(exception.PatronException, self.driver.spawn,
                          self.ctx, instance, image_meta, [], None)
    ...
    def test_destroy_trigger_undeploy_fail(self, fake_validate, mock_sps):
        ...
        instance = fake_instance.fake_instance_obj(self.ctx,
                                                   node=node_uuid)
        mock_sps.side_effect = exception.PatronException()
        self.assertRaises(exception.PatronException, self.driver.destroy,
    ...
    def test_destroy_unassociate_fail(self, mock_node):
        ...
        mock_node.get_by_instance_uuid.return_value = node
        mock_node.update.side_effect = exception.PatronException()
        self.assertRaises(exception.PatronException, self.driver.destroy,
                          self.ctx, instance, None, None)
        mock_node.set_provision_state.assert_called_once_with(node_uuid,
                                                              'deleted')
