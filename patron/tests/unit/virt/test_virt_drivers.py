...
from patron.virt import libvirt
...
class _FakeDriverBackendTestCase(object):
    def _setup_fakelibvirt(self):
        ...
        import patron.tests.unit.virt.libvirt.fake_imagebackend as \
            fake_imagebackend
        import patron.tests.unit.virt.libvirt.fake_libvirt_utils as \
            fake_libvirt_utils
        import patron.tests.unit.virt.libvirt.fakelibvirt as fakelibvirt

        sys.modules['libvirt'] = fakelibvirt
        import patron.virt.libvirt.driver
        import patron.virt.libvirt.firewall
        import patron.virt.libvirt.host

        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.driver.imagebackend',
            fake_imagebackend))
        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.driver.libvirt',
            fakelibvirt))
        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.driver.libvirt_utils',
            fake_libvirt_utils))
        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.host.libvirt',
            fakelibvirt))
        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.imagebackend.libvirt_utils',
            fake_libvirt_utils))
        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.firewall.libvirt',
            fakelibvirt))
        ...
        self.stubs.Set(patron.virt.libvirt.driver.LibvirtDriver,
                       '_get_instance_disk_info',
                       fake_get_instance_disk_info)

        self.stubs.Set(patron.virt.libvirt.driver.disk,
                       'extend', fake_extend)
        ...
        self.stubs.Set(patron.virt.libvirt.driver.libvirt.Domain,
...
class VirtDriverLoaderTestCase(_FakeDriverBackendTestCase, test.TestCase):
    ...
    new_drivers = {
        'patron.virt.fake.FakeDriver': 'FakeDriver',
        'patron.virt.libvirt.LibvirtDriver': 'LibvirtDriver',
    ...
    def test_fail_to_load_new_drivers(self):
        self.flags(compute_driver='patron.virt.amiga')
...
class LibvirtConnTestCase(_VirtDriverTestCase, test.TestCase):
    ...
    def setUp(self):
        ...
        self.driver_module = 'patron.virt.libvirt.LibvirtDriver'
        super(LibvirtConnTestCase, self).setUp()
        self.stubs.Set(self.connection,
                       '_set_host_enabled', mock.MagicMock())
        self.useFixture(fixtures.MonkeyPatch(
            'patron.context.get_admin_context',
