...
from patron.tests.unit import fake_processutils
import patron.tests.unit.image.fake as fake_image
from patron.tests.unit import matchers
from patron.tests.unit.objects import test_aggregate
...
from patron.virt.xenapi import agent
from patron.virt.xenapi.client import session as xenapi_session
from patron.virt.xenapi import driver as xenapi_conn
from patron.virt.xenapi import fake as xenapi_fake
from patron.virt.xenapi import host
from patron.virt.xenapi.image import glance
from patron.virt.xenapi import pool
from patron.virt.xenapi import pool_states
from patron.virt.xenapi import vm_utils
from patron.virt.xenapi import vmops
from patron.virt.xenapi import volume_utils

LOG = logging.getLogger(__name__)

CONF = cfg.CONF
CONF.import_opt('compute_manager', 'patron.service')
CONF.import_opt('network_manager', 'patron.service')
CONF.import_opt('compute_driver', 'patron.virt.driver')
CONF.import_opt('host', 'patron.netconf')
CONF.import_opt('default_availability_zone', 'patron.availability_zones')
CONF.import_opt('login_timeout', 'patron.virt.xenapi.client.session',
                group="xenserver")
...
class XenAPIVolumeTestCase(stubs.XenAPITestBaseNoDB):
    ...
    def setUp(self):
        ...
        self.flags(firewall_driver='patron.virt.xenapi.firewall.'
...
class XenAPIVMTestCase(stubs.XenAPITestBase):
    ...
    def setUp(self):
        ...
        self.flags(instance_name_template='%d',
                   firewall_driver='patron.virt.xenapi.firewall.'
                                   'Dom0IptablesFirewallDriver')
        self.flags(connection_url='test_url',
                   connection_password='test_pass',
                   group='xenserver')
    ...
    def test_init_host(self):
        ...
        vdi1 = xenapi_fake.create_vdi('instance-aaaa', None,
                other_config={'patron_instance_uuid': 'aaaa'})
    ...
    def test_instance_snapshot_fails_with_no_primary_vdi(self):
        ...
        self.assertRaises(exception.PatronException, self.conn.snapshot,
...
"""
    ...
    def test_spawn_vlanmanager(self):
        self.flags(network_manager='patron.network.manager.VlanManager',
        ...
        with mock.patch('patron.objects.network.Network._from_db_object'):
        ...
        # consistent with bridge specified in patron db
    ...
    @mock.patch('patron.db.agent_build_get_by_triple')
...
class XenAPIMigrateInstance(stubs.XenAPITestBase):
    ...
    def setUp(self):
        ...
        migration_values = {
            'source_compute': 'patron-compute',
            'dest_compute': 'patron-compute',
...
class XenAPIHostTestCase(stubs.XenAPITestBase):
    ...
    def test_set_enable_host_enable(self):
        _create_service_entries(self.context, values={'patron': ['fake-mini']})
...
class XenAPIBWCountersTestCase(stubs.XenAPITestBaseNoDB):
    FAKE_VMS = {'test1:ref': dict(name_label='test1',
                                   other_config=dict(patron_uuid='hash'),
                                   domid='12',
                                   _vifmap={'0': "a:b:c:d...",
                                           '1': "e:f:12:q..."}),
...
# patron.tests.unit.virt.test_libvirt.IPTablesFirewallDriverTestCase
...
class XenAPIDom0IptablesFirewallTestCase(stubs.XenAPITestBase):
    ...
    def test_static_filters(self):
        ...
        from patron.compute import utils as compute_utils  # noqa
...
class XenAPISessionTestCase(test.NoDBTestCase):
    ...
    def test_verify_plugin_version_same(self):
        ...
        session.call_plugin_serialized('patron_plugin_version', 'get_version',
                            ).AndReturn("2.4")
