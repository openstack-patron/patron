...
from patron.tests.unit.virt.hyperv import test_base
from patron.virt import hardware
from patron.virt.hyperv import constants
from patron.virt.hyperv import vmops
from patron.virt.hyperv import vmutils
...
class VMOpsTestCase(test_base.HyperVBaseTestCase):
    ...
    @mock.patch('patron.virt.hyperv.vmops.importutils.import_object')
    ...
    @mock.patch('patron.virt.hyperv.imagecache.ImageCache.get_cached_image')
    ...
    @mock.patch('patron.virt.hyperv.vmops.VMOps.destroy')
    @mock.patch('patron.virt.hyperv.vmops.VMOps.power_on')
    @mock.patch('patron.virt.hyperv.vmops.VMOps.attach_config_drive')
    @mock.patch('patron.virt.hyperv.vmops.VMOps._create_config_drive')
    @mock.patch('patron.virt.configdrive.required_by')
    @mock.patch('patron.virt.hyperv.vmops.VMOps.create_instance')
    @mock.patch('patron.virt.hyperv.vmops.VMOps.get_image_vm_generation')
    @mock.patch('patron.virt.hyperv.vmops.VMOps.create_ephemeral_vhd')
    @mock.patch('patron.virt.hyperv.vmops.VMOps._create_root_vhd')
    @mock.patch('patron.virt.hyperv.volumeops.VolumeOps.'
                'ebs_root_in_block_devices')
    @mock.patch('patron.virt.hyperv.vmops.VMOps._delete_disk_files')
    ...
    @mock.patch('patron.api.metadata.base.InstanceMetadata')
    @mock.patch('patron.virt.configdrive.ConfigDriveBuilder')
    ...
    @mock.patch('patron.virt.hyperv.volumeops.VolumeOps.disconnect_volumes')
    @mock.patch('patron.virt.hyperv.vmops.VMOps._delete_disk_files')
    @mock.patch('patron.virt.hyperv.vmops.VMOps.power_off')
    ...
    @mock.patch("patron.virt.hyperv.vmops.VMOps._soft_shutdown")
    ...
    @mock.patch("patron.virt.hyperv.vmops.VMOps._wait_for_power_off")
    ...
    @mock.patch('patron.virt.hyperv.vmops.VMOps._set_vm_state')
    ...
    def test_get_console_output(self, fake_path_exists):
        ...
        with mock.patch('patron.virt.hyperv.vmops.open',
    ...
    def test_get_console_output_exception(self, fake_path_exists, fake_open):
        ...
        with mock.patch('patron.virt.hyperv.vmops.open', fake_open, create=True):
            self.assertRaises(vmutils.HyperVException,
