...
from patron.tests.unit.virt.vmwareapi import fake as vmwareapi_fake
from patron.tests.unit.virt.vmwareapi import stubs
from patron.virt import hardware
from patron.virt.vmwareapi import constants
from patron.virt.vmwareapi import driver
from patron.virt.vmwareapi import ds_util
from patron.virt.vmwareapi import images
from patron.virt.vmwareapi import vim_util
from patron.virt.vmwareapi import vm_util
from patron.virt.vmwareapi import vmops
...
class VMwareVMOpsTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self._image_id = patron.tests.unit.image.fake.get_valid_image_id()
    ...
    @mock.patch('patron.virt.vmwareapi.vm_util.power_on_instance')
    @mock.patch.object(vmops.VMwareVMOps, '_use_disk_image_as_linked_clone')
    @mock.patch.object(vmops.VMwareVMOps, '_fetch_image_if_missing')
    @mock.patch(
        'patron.virt.vmwareapi.imagecache.ImageCacheManager.enlist_image')
    @mock.patch.object(vmops.VMwareVMOps, 'build_virtual_machine')
    @mock.patch.object(vmops.VMwareVMOps, '_get_vm_config_info')
    @mock.patch.object(vmops.VMwareVMOps, '_get_extra_specs')
    @mock.patch.object(patron.virt.vmwareapi.images.VMwareImage,
                       'from_image')
    ...
    @mock.patch(
        'patron.virt.vmwareapi.vmops.VMwareVMOps._configure_config_drive')
    @mock.patch('patron.virt.vmwareapi.ds_util.get_datastore')
    @mock.patch(
        'patron.virt.vmwareapi.vmops.VMwareVMOps.get_datacenter_ref_and_name')
    @mock.patch('patron.virt.vmwareapi.vif.get_vif_info',
                return_value=[])
    @mock.patch('patron.utils.is_neutron',
                return_value=False)
    @mock.patch('patron.virt.vmwareapi.vm_util.get_vm_create_spec',
                return_value='fake_create_spec')
    @mock.patch('patron.virt.vmwareapi.vm_util.create_vm',
                return_value='fake_vm_ref')
    @mock.patch('patron.virt.vmwareapi.ds_util.mkdir')
    @mock.patch('patron.virt.vmwareapi.vmops.VMwareVMOps._set_machine_id')
    @mock.patch(
        'patron.virt.vmwareapi.imagecache.ImageCacheManager.enlist_image')
    @mock.patch.object(vmops.VMwareVMOps, '_get_and_set_vnc_config')
    @mock.patch('patron.virt.vmwareapi.vm_util.power_on_instance')
    @mock.patch('patron.virt.vmwareapi.vm_util.copy_virtual_disk')
    ...
    def test_build_virtual_machine(self):
        image_id = patron.tests.unit.image.fake.get_valid_image_id()
