...
from patron.api.metadata import base as instance_metadata
from patron.compute import arch
from patron.compute import cpumodel
from patron.compute import manager
from patron.compute import power_state
from patron.compute import task_states
from patron.compute import utils as compute_utils
from patron.compute import vm_mode
from patron.compute import vm_states
...
from patron.openstack.common import loopingcall
from patron.pci import manager as pci_manager
from patron import test
from patron.tests.unit import fake_block_device
from patron.tests.unit import fake_instance
from patron.tests.unit import fake_network
import patron.tests.unit.image.fake
from patron.tests.unit import matchers
from patron.tests.unit.objects import test_pci_device
from patron.tests.unit.objects import test_vcpu_model
from patron.tests.unit.virt.libvirt import fake_imagebackend
from patron.tests.unit.virt.libvirt import fake_libvirt_utils
from patron.tests.unit.virt.libvirt import fakelibvirt
from patron import utils
from patron import version
from patron.virt import block_device as driver_block_device
from patron.virt import configdrive
from patron.virt.disk import api as disk
from patron.virt import driver
from patron.virt import fake
from patron.virt import firewall as base_firewall
from patron.virt import hardware
from patron.virt.libvirt import blockinfo
from patron.virt.libvirt import config as vconfig
from patron.virt.libvirt import driver as libvirt_driver
from patron.virt.libvirt import firewall
from patron.virt.libvirt import host
from patron.virt.libvirt import imagebackend
from patron.virt.libvirt import lvm
from patron.virt.libvirt import rbd_utils
from patron.virt.libvirt import utils as libvirt_utils
from patron.virt.libvirt import volume as volume_drivers
...
CONF = cfg.CONF
CONF.import_opt('compute_manager', 'patron.service')
CONF.import_opt('host', 'patron.netconf')
CONF.import_opt('my_ip', 'patron.netconf')
CONF.import_opt('image_cache_subdirectory_name', 'patron.virt.imagecache')
CONF.import_opt('instances_path', 'patron.compute.manager')
...
class CacheConcurrencyTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.imagebackend.libvirt_utils',
            fake_libvirt_utils))
...
class LibvirtConnTestCase(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            'patron.virt.libvirt.driver.libvirt_utils',
            fake_libvirt_utils))
        ...
        self.image_service = patron.tests.unit.image.fake.stub_out_image_service(
                self.stubs)
    ...
    def tearDown(self):
        patron.tests.unit.image.fake.FakeImageService_reset()
    ...
    def create_fake_libvirt_mock(self, **kwargs):
        ...
        # Creating mocks
        volume_driver = ['iscsi=patron.tests.unit.virt.libvirt.test_driver'
                         '.FakeVolumeDriver']
    ...
    def test_set_host_enabled_swallows_exceptions(self):
        ...
        with mock.patch.object(db, 'service_get_by_compute_host') as db_mock:
            # Make db.service_get_by_compute_host raise PatronException; this
            # is more robust than just raising ComputeHostNotFound.
            db_mock.side_effect = exception.PatronException
            drvr._set_host_enabled(False)
    ...
    @mock.patch('patron.console.serial.acquire_port')
    @mock.patch('patron.virt.hardware.get_number_of_serial_ports',
                return_value=1)
    ...
    @mock.patch('patron.virt.disk.api.teardown_container')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver.get_info')
    @mock.patch('patron.virt.disk.api.setup_container')
    @mock.patch('patron.openstack.common.fileutils.ensure_tree')
    ...
    @mock.patch('patron.virt.libvirt.driver.os.path.exists')
    ...
    def _test_get_guest_config_ppc64(self, device_index):
        """Test for patron.virt.libvirt.driver.LibvirtDriver._get_guest_config.
        """
        self.flags(virt_type='kvm', group='libvirt')
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        instance_ref = objects.Instance(**self.test_instance)
    ...
    @mock.patch('patron.virt.libvirt.volume.LibvirtFakeVolumeDriver.'
                'connect_volume')
    @mock.patch('patron.virt.libvirt.volume.LibvirtFakeVolumeDriver.get_config')
    ...
    @mock.patch('patron.utils.get_image_from_system_metadata')
    @mock.patch('patron.virt.libvirt.blockinfo.get_info_from_bdm')
    @mock.patch('patron.virt.libvirt.host.Host.get_domain')
    ...
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._get_disk_xml')
    @mock.patch('patron.virt.libvirt.host.Host.get_domain')
    ...
    def _check_xml_and_uri(self, instance, mock_serial,
        ...
        for virt_type in hypervisors_to_check:
            ...
            with mock.patch('patron.virt.libvirt.driver.libvirt') as old_virt:
    ...
    def test_ensure_filtering_rules_for_instance_timeout(self):
        ...
        try:
            ...
            drvr.ensure_filtering_rules_for_instance(instance_ref,
                                                     network_info)
        except exception.PatronException as e:
    ...
    @mock.patch('patron.virt.libvirt.LibvirtDriver._conn')
    @mock.patch.object(patron.virt.libvirt, 'config')
    ...
    @mock.patch('patron.virt.libvirt.LibvirtDriver._conn')
    @mock.patch.object(patron.virt.libvirt.LibvirtDriver,
    ...
    @mock.patch('patron.virt.libvirt.utils.get_instance_path_at_destination')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver.destroy')
    ...
    @mock.patch('patron.virt.driver.block_device_info_get_mapping',
                return_value=())
    @mock.patch('patron.virt.configdrive.required_by',
                return_value=True)
    ...
    def _test_create_image_plain(self, os_type='', filename='', mkfs=False):
        ...
        self.stubs.Set(patron.virt.libvirt.imagebackend.Backend, "image",
                       fake_image)
        ...
        self.stubs.Set(drvr, 'get_info', fake_get_info)
        if mkfs:
            self.stubs.Set(patron.virt.disk.api, '_MKFS_COMMAND',
    ...
    @mock.patch.object(patron.virt.libvirt.imagebackend.Image, 'cache',
    ...
    @mock.patch('patron.compute.utils.get_machine_ips')
    ...
    @mock.patch('patron.virt.libvirt.LibvirtDriver.get_info')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._create_domain_and_network')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._create_images_and_backing')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._get_guest_xml')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._get_instance_disk_info')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._destroy')
    ...
    @mock.patch('patron.openstack.common.fileutils.ensure_tree')
    @mock.patch('patron.openstack.common.loopingcall.FixedIntervalLoopingCall')
    @mock.patch('patron.pci.manager.get_instance_pci_devs')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._prepare_pci_devices_for_use')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._create_domain_and_network')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._create_images_and_backing')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._get_instance_disk_info')
    @mock.patch('patron.virt.libvirt.utils.write_to_file')
    @mock.patch('patron.virt.libvirt.utils.get_instance_path')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._get_guest_config')
    @mock.patch('patron.virt.libvirt.blockinfo.get_disk_info')
    @mock.patch('patron.virt.libvirt.LibvirtDriver._destroy')
    def test_hard_reboot_does_not_call_glance_show(self,
        ...
        https://bugs.launchpad.net/patron/+bug/1339386
        """
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
...
""")
    ...
    @mock.patch('patron.virt.disk.api.clean_lxc_namespace')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver.get_info')
    @mock.patch('patron.virt.disk.api.setup_container')
    @mock.patch('patron.openstack.common.fileutils.ensure_tree')
    @mock.patch.object(fake_libvirt_utils, 'get_instance_path')
    ...
    def test_create_domain_define_xml_fails(self):
        ...
        self.stubs.Set(patron.virt.libvirt.driver.LOG, 'error', fake_error)
    ...
    def test_create_propagates_exceptions(self):
        ...
        with contextlib.nested(
              ...
              mock.patch.object(drvr, 'firewall_driver'),
              mock.patch.object(drvr, '_create_domain',
                                side_effect=exception.PatronException),
              mock.patch.object(drvr, 'cleanup')):
            self.assertRaises(exception.PatronException,
                              drvr._create_domain_and_network,
                              self.context,
    ...
    @mock.patch('patron.utils.is_neutron', return_value=True)
    ...
    @mock.patch('patron.utils.is_neutron', return_value=False)
    def test_create_with_network_events_non_neutron(self, is_neutron):
        self._test_create_with_network_events()

    @mock.patch('patron.volume.encryptors.get_encryption_metadata')
    @mock.patch('patron.virt.libvirt.blockinfo.get_info_from_bdm')
    ...
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._unplug_vifs')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._undefine_domain')
    def test_cleanup_pass_with_no_mount_device(self, undefine, unplug):
        ...
        fake_bdms = [{'connection_info': 'foo',
                     'mount_device': None}]
        with mock.patch('patron.virt.driver'
    ...
    @mock.patch('patron.virt.driver.block_device_info_get_mapping')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver.'
    ...
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._disconnect_volume')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._swap_volume')
    @mock.patch('patron.virt.block_device.DriverVolumeBlockDevice.save')
    @mock.patch('patron.objects.block_device.BlockDeviceMapping.'
                'get_by_volume_id')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._get_volume_config')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._connect_volume')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._get_disk_xml')
    @mock.patch('patron.virt.libvirt.host.Host.get_domain')
...
class LibvirtDriverTestCase(test.NoDBTestCase):
    """Test for patron.virt.libvirt.libvirt_driver.LibvirtDriver."""
    def setUp(self):
    ...
    def test_migrate_disk_and_power_off_exception(self):
        """Test for patron.virt.libvirt.libvirt_driver.LivirtConnection
    ...
    @mock.patch('patron.utils.execute')
    @mock.patch('patron.virt.libvirt.utils.copy_image')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver._destroy')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver.get_host_ip_addr')
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver'
    ...
    def _test_migrate_disk_and_power_off_resize_check(self, expected_exc):
        """Test for patron.virt.libvirt.libvirt_driver.LibvirtConnection
        .migrate_disk_and_power_off.
        """
    ...
    @mock.patch('patron.virt.libvirt.driver.LibvirtDriver'
                '.get_instance_disk_info')
    @mock.patch('patron.virt.driver.block_device_info_get_ephemerals')
    ...
    @mock.patch('patron.virt.disk.api.extend')
    ...
    @mock.patch('patron.virt.disk.api.can_resize_image')
    @mock.patch('patron.virt.disk.api.is_image_extendable')
    @mock.patch('patron.virt.disk.api.extend')
    ...
    @mock.patch('os.path.exists')
    @mock.patch('patron.virt.libvirt.lvm.list_volumes')
    ...
    @mock.patch('patron.virt.netutils.get_injected_network_template')
    @mock.patch('patron.virt.disk.api.inject_data')
    ...
    @mock.patch(
        'patron.virt.configdrive.ConfigDriveBuilder.add_instance_metadata')
    @mock.patch('patron.virt.configdrive.ConfigDriveBuilder.make_drive')
    ...
    def test_delete_instance_files_resize(self, get_instance_path, exists,
        ...
        patron.utils.execute.side_effect = [Exception(), None]
        exists.side_effect = [False, False, True, False]
    ...
    def test_delete_instance_files_mv_failed(self, get_instance_path, exists,
        ...
        patron.utils.execute.side_effect = Exception()
    ...
    def test_delete_instance_files_concurrent(self, get_instance_path, exists,
        ...
        patron.utils.execute.side_effect = [Exception(), Exception(), None]
        exists.side_effect = [False, False, True, False]
    ...
    @mock.patch("patron.compute.utils.get_image_metadata")
...
class LibvirtNonblockingTestCase(test.NoDBTestCase):
    ...
    def test_connection_to_primitive(self):
        # Test bug 962840.
        import patron.virt.libvirt.driver as libvirt_driver
...
class LibvirtVolumeSnapshotTestCase(test.NoDBTestCase):
    ...
    @mock.patch('patron.virt.block_device.DriverVolumeBlockDevice.'
    ...
    def test_volume_snapshot_create_outer_failure(self):
        ...
        self.drvr._volume_snapshot_create(self.c,
                                          ...
                                          self.volume_uuid,
                                          self.create_info['new_file']).\
            AndRaise(exception.PatronException('oops'))
...
class LibvirtSnapshotTests(_BaseSnapshotTests):
    def test_ami(self):
        # Assign different image_ref from patron/images/fakes for testing ami
...
class LVMSnapshotTests(_BaseSnapshotTests):
    ...
    def _test_lvm_snapshot(self, disk_format, mock_volume_info,
        ...
        self.flags(images_type='lvm',
                   images_volume_group='patron-vg', group='libvirt')

        self._test_snapshot(disk_format=disk_format)

        mock_volume_info.assert_has_calls([mock.call('/dev/patron-vg/lv')])
        mock_convert_image.assert_called_once_with(
                '/dev/patron-vg/lv', mock.ANY, disk_format, run_as_root=True)
