...
class FakeLibvirtTests(test.NoDBTestCase):
    ...
    def test_nwfilter_define_undefine(self):
        ...
        xml = '''<filter name='patron-instance-instance-789' chain='root'>
                    <uuid>946878c6-3ad3-82b2-87f3-c709f3807f58</uuid>
                 </filter>'''

        conn.nwfilterDefineXML(xml)
        nwfilter = conn.nwfilterLookupByName('patron-instance-instance-789')
        nwfilter.undefine()
        try:
            conn.nwfilterLookupByName('patron-instance-instance-789320334')
        except libvirt.libvirtError as e:
