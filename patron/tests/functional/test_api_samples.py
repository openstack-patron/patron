...
from patron.api.openstack.compute.contrib import fping
from patron.api.openstack.compute import extensions
from patron.cells import utils as cells_utils
# Import extensions to pull in osapi_compute_extension CONF option used below.
from patron.cloudpipe import pipelib
from patron.compute import api as compute_api
from patron.compute import cells_api as cells_api
from patron.compute import manager as compute_manager
from patron.compute import rpcapi as compute_rpcapi
from patron.compute import vm_states
from patron.conductor import manager as conductor_manager
from patron.console import manager as console_manager  # noqa - only for cfg
from patron import context
from patron import db
from patron import exception
from patron.network import api as network_api
from patron.network.neutronv2 import api as neutron_api  # noqa - only for cfg
from patron import objects
from patron.servicegroup import api as service_group_api
from patron import test
from patron.tests.functional import api_samples_test_base
from patron.tests.functional import integrated_helpers
from patron.tests.unit.api.openstack.compute.contrib import test_fping
from patron.tests.unit.api.openstack.compute.contrib import test_networks
from patron.tests.unit.api.openstack.compute.contrib import test_services
...
from patron.tests.unit import fake_network_cache_model
from patron.tests.unit import fake_utils
from patron.tests.unit.image import fake
from patron.tests.unit.objects import test_network
from patron.tests.unit import utils as test_utils
from patron import utils
from patron.volume import cinder

CONF = cfg.CONF
CONF.import_opt('allow_resize_to_same_host', 'patron.compute.api')
CONF.import_opt('shelved_offload_time', 'patron.compute.manager')
CONF.import_opt('enable_network_quota',
                'patron.api.openstack.compute.contrib.os_tenant_networks')
CONF.import_opt('osapi_compute_extension',
                'patron.api.openstack.compute.extensions')
CONF.import_opt('vpn_image_id', 'patron.cloudpipe.pipelib')
CONF.import_opt('osapi_compute_link_prefix', 'patron.api.openstack.common')
CONF.import_opt('osapi_glance_link_prefix', 'patron.api.openstack.common')
CONF.import_opt('enable', 'patron.cells.opts', group='cells')
CONF.import_opt('cell_type', 'patron.cells.opts', group='cells')
CONF.import_opt('db_check_interval', 'patron.cells.state', group='cells')
...
class ServersSampleHideAddressesJsonTest(ServersSampleJsonTest):
    sample_dir = None
    extension_name = '.'.join(('patron.api.openstack.compute.contrib',
...
class ServerStartStopJsonTest(ServersSampleBase):
    extension_name = "patron.api.openstack.compute.contrib" + \
...
class UserDataJsonTest(ApiSampleTestBaseV2):
    extension_name = "patron.api.openstack.compute.contrib.user_data.User_data"
...
class FlavorRxtxJsonTest(ApiSampleTestBaseV2):
    ADMIN_API = True
    extension_name = ('patron.api.openstack.compute.contrib.flavor_rxtx.'
                      'Flavor_rxtx')
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'patron.api.openstack.compute.contrib.flavormanage.Flavormanage')
        return f
...
class SchedulerHintsJsonTest(ApiSampleTestBaseV2):
    extension_name = ("patron.api.openstack.compute.contrib.scheduler_hints."
                     "Scheduler_hints")
...
class KeyPairsSampleJsonTest(ApiSampleTestBaseV2):
    extension_name = "patron.api.openstack.compute.contrib.keypairs.Keypairs"
...
class ExtendedRescueWithImageJsonTest(ServersSampleBase):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'patron.api.openstack.compute.contrib.rescue.Rescue')
        return f
...
class ShelveJsonTest(ServersSampleBase):
    extension_name = "patron.api.openstack.compute.contrib.shelve.Shelve"
...
class CloudPipeSampleJsonTest(ApiSampleTestBaseV2):
    ADMIN_API = True
    extension_name = "patron.api.openstack.compute.contrib.cloudpipe.Cloudpipe"
...
class CloudPipeUpdateJsonTest(ApiSampleTestBaseV2):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'patron.api.openstack.compute.contrib.cloudpipe.Cloudpipe')
        return f
...
class FixedIpJsonTest(ApiSampleTestBaseV2):
    extension_name = "patron.api.openstack.compute.contrib.fixed_ips.Fixed_ips"
...
class UsedLimitsSamplesJsonTest(ApiSampleTestBaseV2):
    extension_name = ("patron.api.openstack.compute.contrib.used_limits."
                      "Used_limits")
...
class UsedLimitsForAdminSamplesJsonTest(ApiSampleTestBaseV2):
    ADMIN_API = True
    extends_name = ("patron.api.openstack.compute.contrib.used_limits."
                    "Used_limits")
    extension_name = (
        "patron.api.openstack.compute.contrib.used_limits_for_admin."
...
class MultipleCreateJsonTest(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.multiple_create."
                      "Multiple_create")
...
class ServicesJsonTest(ApiSampleTestBaseV2):
    extension_name = "patron.api.openstack.compute.contrib.services.Services"
    ADMIN_API = True
...
class ExtendedServicesDeleteJsonTest(ApiSampleTestBaseV2):
    ADMIN_API = True
    extends_name = ("patron.api.openstack.compute.contrib.services.Services")
    extension_name = ("patron.api.openstack.compute.contrib."
...
class SimpleTenantUsageSampleJsonTest(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.simple_tenant_usage."
                      "Simple_tenant_usage")
...
class ServerDiagnosticsSamplesJsonTest(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.server_diagnostics."
                      "Server_diagnostics")
...
class AvailabilityZoneJsonTest(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.availability_zone."
                      "Availability_zone")
...
class AdminActionsSamplesJsonTest(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.admin_actions."
                      "Admin_actions")
    ...
    @mock.patch('patron.conductor.manager.ComputeTaskManager._cold_migrate')
...
class ConsoleAuthTokensSampleJsonTests(ServersSampleBase):
    ADMIN_API = True
    extends_name = ("patron.api.openstack.compute.contrib.consoles.Consoles")
    extension_name = ("patron.api.openstack.compute.contrib.console_auth_tokens."
                      "Console_auth_tokens")
...
class QuotasSampleJsonTests(ApiSampleTestBaseV2):
    ADMIN_API = True
    extension_name = "patron.api.openstack.compute.contrib.quotas.Quotas"
...
class ExtendedQuotasSampleJsonTests(ApiSampleTestBaseV2):
    ADMIN_API = True
    extends_name = "patron.api.openstack.compute.contrib.quotas.Quotas"
    extension_name = ("patron.api.openstack.compute.contrib"
...
class ExtendedVIFNetSampleJsonTests(ServersSampleBase):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            ('patron.api.openstack.compute.contrib'
             '.virtual_interfaces.Virtual_interfaces'))
        return f
...
class FlavorManageSampleJsonTests(ApiSampleTestBaseV2):
    ADMIN_API = True
    extension_name = ("patron.api.openstack.compute.contrib.flavormanage."
                      "Flavormanage")
...
class ServerPasswordSampleJsonTests(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.server_password."
                      "Server_password")
...
class DiskConfigJsonTest(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.disk_config."
                      "Disk_config")
...
class ExtendedNetworksJsonTests(ApiSampleTestBaseV2):
    ADMIN_API = True
    extends_name = ("patron.api.openstack.compute.contrib."
                    "os_networks.Os_networks")
    extension_name = ("patron.api.openstack.compute.contrib."
...
class NetworksAssociateJsonTests(ApiSampleTestBaseV2):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'patron.api.openstack.compute.contrib.os_networks.Os_networks')
        return f
...
class BlockDeviceMappingV2BootJsonTest(ServersSampleBase):
    ...
    def _get_flags(self):
        ...
        f['osapi_compute_extension'].append(
            'patron.api.openstack.compute.contrib.volumes.Volumes')
        return f
...
class MultinicSampleJsonTest(ServersSampleBase):
    ADMIN_API = True
    extension_name = "patron.api.openstack.compute.contrib.multinic.Multinic"

    def _disable_instance_dns_manager(self):
        # NOTE(markmc): it looks like multinic and instance_dns_manager are
        #               incompatible. See:
        #               https://bugs.launchpad.net/patron/+bug/1213251
        self.flags(
            instance_dns_manager='patron.network.noop_dns_driver.NoopDNSDriver')
...
class FlavorExtraSpecsSampleJsonTests(ApiSampleTestBaseV2):
    ADMIN_API = True
    extension_name = ("patron.api.openstack.compute.contrib.flavorextraspecs."
                      "Flavorextraspecs")
...
class FpingSampleJsonTests(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.fping.Fping")
...
class EvacuateFindHostSampleJsonTest(ServersSampleBase):
    ...
    @mock.patch('patron.compute.manager.ComputeManager._check_instance_exists')
    @mock.patch('patron.compute.api.HostAPI.service_get_by_compute_host')
    @mock.patch('patron.conductor.manager.ComputeTaskManager.rebuild_instance')
...
class ConfigDriveSampleJsonTest(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.config_drive."
                      "Config_drive")
...
class FlavorAccessSampleJsonTests(ApiSampleTestBaseV2):
    ADMIN_API = True
    extension_name = ("patron.api.openstack.compute.contrib.flavor_access."
                      "Flavor_access")
...
class HypervisorsSampleJsonTests(ApiSampleTestBaseV2):
    ADMIN_API = True
    extension_name = ("patron.api.openstack.compute.contrib.hypervisors."
                      "Hypervisors")
...
@mock.patch("patron.servicegroup.API.service_is_up", return_value=True)
...
class AttachInterfacesSampleJsonTest(ServersSampleBase):
    extension_name = ('patron.api.openstack.compute.contrib.attach_interfaces.'
                      'Attach_interfaces')
...
class SnapshotsSampleJsonTests(ApiSampleTestBaseV2):
    extension_name = "patron.api.openstack.compute.contrib.volumes.Volumes"
...
class VolumeAttachUpdateSampleJsonTest(VolumeAttachmentsSampleBase):
    extends_name = ("patron.api.openstack.compute.contrib.volumes.Volumes")
    extension_name = ("patron.api.openstack.compute.contrib."
...
class ServerSortKeysJsonTests(ServersSampleBase):
    extension_name = ("patron.api.openstack.compute.contrib.server_sort_keys"
                      ".Server_sort_keys")
