...
class PatronException(Exception):
    """Base Nova Exception
    ...
    def __init__(self, message=None, **kwargs):
        ...
        super(PatronException, self).__init__(message)

    def format_message(self):
        # NOTE(mrodden): use the first argument to the python Exception object
        # which should be our full PatronException message, (see __init__)
        return self.args[0]


class EncryptionFailure(PatronException):
    msg_fmt = _("Failed to encrypt text: %(reason)s")


class DecryptionFailure(PatronException):
    msg_fmt = _("Failed to decrypt text: %(reason)s")


class RevokeCertFailure(PatronException):
    msg_fmt = _("Failed to revoke certificate for %(project_id)s")


class VirtualInterfaceCreateException(PatronException):
    msg_fmt = _("Virtual Interface creation failed")


class VirtualInterfaceMacAddressException(PatronException):
    msg_fmt = _("Creation of virtual interface with "
                "unique mac address failed")


class VirtualInterfacePlugException(PatronException):
    msg_fmt = _("Virtual interface plugin failed")


class GlanceConnectionFailed(PatronException):
    msg_fmt = _("Connection to glance host %(host)s:%(port)s failed: "
        "%(reason)s")


class CinderConnectionFailed(PatronException):
    msg_fmt = _("Connection to cinder host failed: %(reason)s")


class Forbidden(PatronException):
...
class ImageNotActive(PatronException):
...
class ImageNotAuthorized(PatronException):
    msg_fmt = _("Not authorized for image %(image_id)s.")


class Invalid(PatronException):
...
class VolumeNotCreated(PatronException):
...
class HypervisorUnavailable(PatronException):
...
class ComputeServiceInUse(PatronException):
...
class ConstraintNotMet(PatronException):
    msg_fmt = _("Constraint not met.")
    code = 412


class NotFound(PatronException):
...
class AgentBuildExists(PatronException):
...
class ImageNotFoundEC2(ImageNotFound):
    msg_fmt = _("Image %(image_id)s could not be found. The patron EC2 API "
...
class NetworkDhcpReleaseFailed(PatronException):
    msg_fmt = _("Failed to release IP %(address)s with MAC %(mac_address)s")


class NetworkInUse(PatronException):
    msg_fmt = _("Network %(network_id)s is still in use.")


class NetworkSetHostFailed(PatronException):
...
class DuplicateVlan(PatronException):
    msg_fmt = _("Detected existing vlan with id %(vlan)d")
    code = 409


class CidrConflict(PatronException):
...
class NoMoreNetworks(PatronException):
...
class NetworkMissingPhysicalNetwork(PatronException):
...
class FixedIpExists(PatronException):
...
class FixedIpAssociateFailed(PatronException):
    msg_fmt = _("Fixed IP associate failed for network: %(net)s.")


class FixedIpAlreadyInUse(PatronException):
    msg_fmt = _("Fixed IP address %(address)s is already in use on instance "
                "%(instance_uuid)s.")


class FixedIpAssociatedWithMultipleInstances(PatronException):
...
class NoMoreFixedIps(PatronException):
    ec2_code = 'UnsupportedOperation'
...
class FloatingIpExists(PatronException):
...
class FloatingIpMultipleFoundForAddress(PatronException):
...
class FloatingIpAssociated(PatronException):
    ec2_code = "UnsupportedOperation"
    msg_fmt = _("Floating ip %(address)s is associated.")


class FloatingIpNotAssociated(PatronException):
...
class FloatingIpAllocateFailed(PatronException):
    msg_fmt = _("Floating IP allocate failed.")


class CannotDisassociateAutoAssignedFloatingIP(PatronException):
    ec2_code = "UnsupportedOperation"
...
class ServiceBinaryExists(PatronException):
    msg_fmt = _("Service with host %(host)s binary %(binary)s exists.")


class ServiceTopicExists(PatronException):
...
class QuotaExists(PatronException):
...
class OverQuota(PatronException):
...
class NoUniqueMatch(PatronException):
...
class ConsolePoolExists(PatronException):
...
class ConsolePortRangeExhausted(PatronException):
...
class FlavorExtraSpecUpdateCreateFailed(PatronException):
...
class CellExists(PatronException):
    msg_fmt = _("Cell with name %(name)s already exists.")


class CellRoutingInconsistency(PatronException):
...
class CellMaxHopCountReached(PatronException):
    msg_fmt = _("Cell message has reached maximum hop count: %(hop_count)s")


class NoCellsAvailable(PatronException):
    msg_fmt = _("No cells available matching scheduling criteria.")


class CellsUpdateUnsupported(PatronException):
...
class NotAllowed(PatronException):
...
class MalformedRequestBody(PatronException):
...
class AggregateError(PatronException):
...
class AggregateNameExists(PatronException):
...
class AggregateHostExists(PatronException):
    msg_fmt = _("Aggregate %(aggregate_id)s already has host %(host)s.")


class FlavorCreateFailed(PatronException):
    msg_fmt = _("Unable to create flavor")


class InstancePasswordSetFailed(PatronException):
...
class CouldNotFetchImage(PatronException):
    msg_fmt = _("Could not fetch image %(image_id)s")


class CouldNotUploadImage(PatronException):
    msg_fmt = _("Could not upload image %(image_id)s")


class TaskAlreadyRunning(PatronException):
    msg_fmt = _("Task %(task_name)s is already running on host %(host)s")


class TaskNotRunning(PatronException):
...
class ConfigDriveMountFailed(PatronException):
    msg_fmt = _("Could not mount vfat config drive. %(operation)s failed. "
                "Error: %(error)s")


class ConfigDriveUnknownFormat(PatronException):
...
class InstanceUserDataTooLarge(PatronException):
    msg_fmt = _("User data too large. User data must be no larger than "
                "%(maxsize)s bytes once base64 encoded. Your data is "
                "%(length)d bytes")


class InstanceUserDataMalformed(PatronException):
    msg_fmt = _("User data needs to be valid base 64.")


class UnexpectedTaskStateError(PatronException):
...
class ServiceGroupUnavailable(PatronException):
    msg_fmt = _("The service from servicegroup driver %(driver)s is "
                "temporarily unavailable.")


class DBNotAllowed(PatronException):
...
class UnsupportedObjectError(PatronException):
    msg_fmt = _('Unsupported object type %(objtype)s')


class OrphanedObjectError(PatronException):
    msg_fmt = _('Cannot call %(method)s on orphaned %(objtype)s object')


class IncompatibleObjectVersion(PatronException):
    msg_fmt = _('Version %(objver)s of %(objname)s is not supported')


class ReadOnlyFieldError(PatronException):
    msg_fmt = _('Cannot modify readonly field %(field)s')


class ObjectActionError(PatronException):
    msg_fmt = _('Object action %(action)s failed because: %(reason)s')


class ObjectFieldInvalid(PatronException):
    msg_fmt = _('Field %(field)s of %(objname)s is not an instance of Field')


class CoreAPIMissing(PatronException):
    msg_fmt = _("Core API extensions are missing: %(missing_apis)s")


class AgentError(PatronException):
...
class InstanceGroupIdExists(PatronException):
...
class PluginRetriesExceeded(PatronException):
    msg_fmt = _("Number of retries to plugin (%(num_retries)d) exceeded.")


class ImageDownloadModuleError(PatronException):
...
class PciDeviceRequestFailed(PatronException):
    msg_fmt = _(
        "PCI device request (%requests)s failed")


class PciDevicePoolEmpty(PatronException):
    msg_fmt = _(
...
class PciRequestAliasNotDefined(PatronException):
    msg_fmt = _("PCI alias %(alias)s is not defined")


class MissingParameter(PatronException):
...
class InternalError(PatronException):
    ec2_code = 'InternalError'
    msg_fmt = "%(err)s"


class PciDevicePrepareFailed(PatronException):
    msg_fmt = _("Failed to prepare PCI device %(id)s for instance "
                "%(instance_uuid)s: %(reason)s")


class PciDeviceDetachFailed(PatronException):
    msg_fmt = _("Failed to detach PCI device %(dev)s: %(reason)s")


class PciDeviceUnsupportedHypervisor(PatronException):
    msg_fmt = _("%(type)s hypervisor does not support PCI devices")


class KeyManagerError(PatronException):
...
class RequestedVRamTooHigh(PatronException):
...
class NoLiveMigrationForConfigDriveInLibVirt(PatronException):
...
class LiveMigrationWithOldNovaNotSafe(PatronException):
...
class UnshelveException(PatronException):
...
class SocketPortRangeExhaustedException(PatronException):
    msg_fmt = _("Not able to acquire a free port for %(host)s")


class SocketPortInUseException(PatronException):
