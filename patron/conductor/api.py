...
from patron.conductor import manager
from patron.conductor import rpcapi
...
conductor_opts = [
    cfg.BoolOpt('use_local',
                default=False,
                help='Perform patron-conductor operations locally'),
    cfg.StrOpt('topic',
               default='conductor',
               help='The topic on which conductor nodes listen'),
    cfg.StrOpt('manager',
               default='patron.conductor.manager.ConductorManager',
...
class API(LocalAPI):
    ...
    def wait_until_ready(self, context, early_timeout=10, early_attempts=10):
        ...
        while True:
            ...
            # This may fail the first time around if patron-conductor wasn't
            # running when this service started.
            try:
                self.base_rpcapi.ping(context, '1.21 GigaWatts',
                                      timeout=timeout)
                if has_timedout:
                    LOG.info(_LI('patron-conductor connection '
                                 'established successfully'))
                break
            except messaging.MessagingTimeout:
                has_timedout = True
                LOG.warning(_LW('Timed out waiting for patron-conductor.  '
                                'Is it running? Or did this service start '
                                'before patron-conductor?  '
                                'Reattempting establishment of '
                                'patron-conductor connection...'))
...
class ComputeTaskAPI(object):
    """ComputeTask API that queues up compute tasks for patron-conductor."""
