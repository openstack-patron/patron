...
from patron.objects import base as patron_object
...
class ConductorManager(manager.Manager):
    ...
    The methods in the base API for patron-conductor are various proxy operations
    performed on behalf of the patron-compute service running on compute nodes.
    Compute nodes are not allowed to directly access the database, so this set
    of methods allows them to get specific work done without locally accessing
    the database.

    The patron-conductor service also exposes an API in the 'compute_task'
    ...
    def object_class_action(self, context, objname, objmethod,
        ...
        objclass = patron_object.NovaObject.obj_class_from_name(objname,
                                                              objver)
        ...
        return (result.obj_to_primitive(target_version=objver)
                if isinstance(result, patron_object.NovaObject) else result)
...
class ComputeTaskManager(base.Base):
    ...
    This class presents an rpc API for patron-conductor under the 'compute_task'
